...
public class StreamController(IStreamService streamService, IThumbnailService thumbnailService, ITranscodeService transcodeService, CoveContext db) : ControllerBase
{
    ...
    private const double DefaultDetectionCropContext = DetectionCropRenderer.DefaultContext;
    ...
    {
        var output = await DetectionCropRenderer.RenderAsync(detection, sourceStream, max, context, ct);
        if (output is null)
            return NotFound();
