...
function createNestedEntityNavigationHandlers<T extends HTMLAnchorElement>(
...
export function CardExtensionSlot<TContext extends object>({ slot, context }: { slot: string; context: TContext }) {
  ...
  if (!has) return null;
  return (
    <div className="card-extension relative isolate max-h-24 overflow-hidden" style={{ contain: "layout paint" }}>
  ...
  return (
    ...
    >
      <RouteCardLinkOverlay
        ...
        label={label}
        disabled={selecting}
        selectionSafeZone={selectable && (selected !== undefined || selecting)}
        onMouseMove={onOverlayMouseMove ? (event) => onOverlayMouseMove(event.clientX, event.clientY) : undefined}
        onMouseLeave={onOverlayMouseLeave}
      />
      <div
        className={`card-media relative flex shrink-0 items-center justify-center overflow-hidden ${mediaClassName}`}
      >
        {media}
        {selectable && (selected !== undefined || selecting) ? (
      ...
      {children && extensionBeforeFooter ? (
        <div className={`relative z-10 ${extensionClassName}`}>{children}</div>
      ) : null}
      {footer ? (
        <>
...
export function CardFavoriteButton(props: { hostType: AffinityHostType; hostId: number; favorite: boolean }) {
  ...
  return (
    <span
      className="inline-flex min-h-7 items-center justify-center p-1 text-red-400"
      title="Favorite"
      aria-label="Favorite"
    >
...
export function PerformerPreviewGrid({
  ...
  return (
    <div className="grid grid-cols-2 gap-2">
      {performerItems.map((performer) => {
        const navigationHandlers = createNestedEntityNavigationHandlers<HTMLAnchorElement>(
        ...
        return (
          ...
          >
            ...
            <span className="text-xs text-accent group-hover/perf:underline truncate w-full font-medium">
              {performer.name}
            </span>
          </a>
        );
...
export function GalleryPreviewList({
  galleries: galleryItems,
  onNavigate,
}: {
  galleries: Array<{ id: number; title?: string | null; date?: string | null; coverPath?: string | null }>;
  onNavigate?: (route: any) => void;
}) {
  return (
...
function EntityLinkList({
  ...
  items: Array<{ id: number; label: string; color?: string | null }>;
  page: string;
  onNavigate?: (route: any) => void;
}) {
  return (
      ...
      {items.map((item) => {
        ...
        return (
          <a
            key={`${page}-${item.id}`}
            {...navigationHandlers}
            className="flex items-center gap-2 rounded px-1.5 py-1 text-xs font-medium text-accent transition-colors hover:bg-card-hover hover:underline"
          >
  ...
  return (
    <div data-studio-overlay className="absolute right-0 top-0 z-[5] p-1">
      <a {...navigationHandlers} aria-label={studioName} className="block">
        ...
        />
        <span className="rounded bg-black/60 px-1.5 py-0.5 text-xs font-medium text-white" style={{ display: "none" }}>
          {studioName}
        </span>
      </a>
    </div>
...
function MediaCardPerformerBadges({
  performerItems,
  onNavigate,
}: {
  performerItems: PerformerSummary[];
  onNavigate?: (route: any) => void;
}) {
    ...
    <div className="relative z-10 flex flex-wrap items-center gap-1.5 overflow-hidden">
      {performerItems.slice(0, 4).map((performer) => (
        <PerformerBadge
          key={performer.id}
          performer={performer}
          navigationHandlers={createNestedEntityNavigationHandlers<HTMLAnchorElement>(
            { page: "performer", id: performer.id },
            onNavigate,
          )}
        />
...
function AudioTextCardPopovers({
  hostType,
  hostId,
  performers: performerItems,
  tags: tagItems,
  ...
  return (
    <>
      ...
      <div className="card-popovers relative z-10 flex min-h-[28px] flex-wrap items-center justify-center gap-1 rounded-b px-2 py-1.5">
        ...
        {groupItems.length > 0 ? (
          <PopoverButton icon={<Layers className="h-3.5 w-3.5" />} count={groupItems.length} title="Groups" preferBelow>
            <EntityLinkList
        ...
        {organized ? (
          <span className="p-1 text-muted" title="Organized">
    ...
    <div className="space-y-1">
      {items.map((s) => (
        <div key={s.id} className="flex items-center gap-2 px-1 py-0.5 rounded hover:bg-card">
          <img
            src={videos.screenshotUrl(s.id, s.updatedAt)}
            alt=""
            className="w-12 h-7 rounded object-cover flex-shrink-0 bg-surface"
            loading="lazy"
            onError={(e) => {
    ...
    <div className="grid grid-cols-3 gap-1">
      {items.map((img) => (
        <div key={img.id} className="aspect-square rounded overflow-hidden bg-surface">
          <CoverImage
            src={images.thumbnailUrl(img.id)}
            alt=""
...
export function AudiosPopoverContent({ filter }: { filter: AudioFilterCriteria }) {
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      audios.findFiltered({ findFilter: { perPage: 10, sort: "created_at", direction: "desc" }, objectFilter: filter }),
  });
...
export function TextsPopoverContent({ filter }: { filter: TextFilterCriteria }) {
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      texts.findFiltered({ findFilter: { perPage: 10, sort: "created_at", direction: "desc" }, objectFilter: filter }),
  });
  if (isLoading) return <p className="text-[11px] text-muted px-1">Loading...</p>;
  const items = data?.items ?? [];
  ...
  return (
    <div className="space-y-1">
      {items.map((s) => {
        ...
        return (
          <a key={s.id} {...navigationHandlers} className="flex items-center gap-2 px-1 py-0.5 rounded hover:bg-card">
            {s.imagePath ? (
              <img
                src={s.imagePath}
                alt=""
                className="w-10 h-7 rounded object-contain flex-shrink-0 bg-surface"
                loading="lazy"
                onError={(e) => {
                  (e.target as HTMLImageElement).style.display = "none";
                }}
    ...
    <div className="space-y-1">
      {items.map((g) => (
        <div key={g.id} className="flex items-center gap-2 px-1 py-0.5 rounded hover:bg-card">
          {g.frontImagePath ? (
            <CoverImage
              src={g.frontImagePath}
              alt=""
              className="w-7 h-10 rounded flex-shrink-0 bg-surface"
              loading="lazy"
              onError={(e) => {
                (e.target as HTMLImageElement).style.display = "none";
              }}
            ...
            <Layers className="w-4 h-4 text-muted flex-shrink-0" />
          )}
...
export function VideoCardPopovers({
  video,
  engagement,
  onNavigate,
}: {
  ...
  return (
    <>
      ...
      <div className="relative z-10 flex flex-wrap items-center justify-center gap-1 px-2 py-1.5 rounded-b card-popovers min-h-[28px]">
        ...
        {likeCount > 0 && <LikeCounter count={likeCount} />}
        {hasFavorite ? (
        ...
        {video.groups.length > 0 && (
          <PopoverButton
            ...
            count={video.groups.length}
            title="Groups"
            preferBelow
          >
            <EntityLinkList
              items={video.groups.map((group: any) => ({ id: group.id, label: group.name }))}
              page="group"
              onNavigate={onNavigate}
            />
          </PopoverButton>
        ...
        {video.galleries.length > 0 && (
          <PopoverButton
            icon={<ImagesIcon className="w-3.5 h-3.5" />}
            count={video.galleries.length}
            title="Galleries"
            preferBelow
          >
            <EntityLinkList
              items={video.galleries.map((gallery: any) => ({ id: gallery.id, label: gallery.title || "Untitled" }))}
              page="gallery"
              onNavigate={onNavigate}
            />
          </PopoverButton>
...
export function StudioHeaderImage({
  studioId,
  studioName,
  onNavigate,
}: {
...
export function MediaStudioSubtitle({
  date,
  studioId,
  studioName,
  fieldProvenance,
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 text-sm text-secondary">
      {date ? (
        <FieldProvenanceHover fieldProvenance={fieldProvenance} fieldKey="date">
...
export function VideoCard({
  ...
  const visibleResumeTime =
    typeof video.clipStartSec === "number" && typeof engagement?.resumeTime === "number"
  ...
  return (
    <div
      onClick={selecting ? (event) => onClick(toggleOptionsFromEvent(event)) : undefined}
      className={`video-card relative cursor-pointer group rounded border bg-card overflow-hidden flex flex-col h-full ${selected ? "ring-2 ring-accent border-accent" : "border-border"}`}
    >
      <RouteCardLinkOverlay
        route={{ page: "video", id: video.id }}
        onClick={onClick}
        label={`Open video ${cardTitle}`}
        disabled={selecting}
      ...
      <VideoPreviewThumbnail video={video} fit={videoPreviewObjectFit} enableScrubbing={!selecting}>
        ...
        {(duration > 0 || resLabel) && (
          <div className="video-specs-overlay absolute bottom-0 right-0 flex items-center gap-0.5 px-1.5 py-1 text-xs text-white z-[5] transition-opacity">
            {file && (
              <span className="bg-black/70 px-1 py-0.5 rounded extra-video-info hidden">
      ...
      <div className="card-body px-2.5 pt-2 pb-2 border-t border-border/50 flex-1 flex flex-col gap-1.5 min-h-0">
        <div>
          <p
            className="card-title font-semibold text-foreground line-clamp-2 group-hover:text-accent transition-colors leading-snug"
            title={cardTitle}
          >
            {cardTitle}
          </p>
  ...
  return (
    <a {...linkProps} className="group text-left">
      <div className="relative aspect-video overflow-hidden rounded-lg border border-border bg-card shadow-md shadow-black/30">
        ...
        {resLabel && (
          <span className="absolute top-1.5 right-1.5 rounded bg-black/75 px-1.5 py-0.5 text-[10px] font-bold uppercase text-accent">
            {resLabel}
          </span>
        )}
      ...
      <div className="pt-2">
        <p className="card-title font-medium text-foreground line-clamp-2 group-hover:text-accent">
...
function PerformerGenderIcon({ gender }: { gender?: string }) {
  ...
  return (
    <span
      className={`inline-flex ${definition.colorClass ?? ""}`.trim()}
...
export function PerformerTile({
  ...
  selecting,
  referenceDate,
}: PerformerTileProps & { engagement?: EntityEngagement }) {
  ...
  const ageTooltip = performer.birthdate
    ? currentAge != null
      ? isDeceased
        ? `${performer.birthdate}, ${currentAge} years old at death`
        : `${performer.birthdate}, now ${currentAge} years old`
      : performer.birthdate
    : undefined;
    ...
    <EntityTileFrame
      ...
      media={
        <>
          ...
          {performer.favorite ? (
            <Heart className="absolute right-1.5 top-1.5 z-[5] h-4 w-4 fill-red-500 text-red-500 drop-shadow-md" />
          ) : null}
        </>
      }
      body={
        <>
          <div className="flex min-w-0 items-start gap-2">
            <p className="card-title min-w-0 flex-1 line-clamp-2 font-semibold text-foreground group-hover:text-accent">
              {performer.name}
            </p>
            {performer.country ? (
              selecting ? (
                <CountryFlag
                  ...
                  className="pointer-events-none shrink-0 text-base leading-none drop-shadow"
                />
              ) : (
                <a
                  {...createRouteLinkProps<HTMLAnchorElement>({ page: "performer", id: performer.id }, () => onClick())}
                  tabIndex={-1}
                  className="relative z-10 shrink-0 text-base leading-none drop-shadow"
                >
              ...
              {age != null ? (
                selecting ? (
                  <span
                    ...
                    title={ageTooltip}
                    aria-label={`${age} years old; ${ageTooltip}`}
                  >
                    {age} years old
                  </span>
                ) : (
                  <a
                    {...createRouteLinkProps<HTMLAnchorElement>({ page: "performer", id: performer.id }, () =>
                      onClick(),
                    )}
                    tabIndex={-1}
                    className="relative z-10 whitespace-nowrap"
          ...
          <>
            ...
            {audioCount > 0 && (
              <PopoverButton
                ...
                preferBelow
              >
                <AudiosPopoverContent
                  filter={{ performersCriterion: { modifier: "INCLUDES", value: [performer.id] } }}
                />
              </PopoverButton>
            )}
            {textCount > 0 && (
            ...
            {groupCount > 0 ? (
              <span className="flex items-center gap-0.5 text-xs text-muted px-1" title="Groups">
                <Layers className="w-3 h-3" /> {groupCount}
              </span>
            ) : null}
            {likeCount > 0 ? <LikeCounter count={likeCount} /> : null}
          </>
...
export function StudioTile({
  ...
  onSelect,
  selecting,
}: StudioTileProps & { engagement?: EntityEngagement }) {
  const hasFooter =
          ...
          <>
            {studio.tags.length > 0 ? (
              <PopoverButton icon={<Tag className="w-3.5 h-3.5" />} count={studio.tags.length} title="Tags" preferBelow>
                <EntityLinkList
                  items={studio.tags.map((tag) => ({
                    id: tag.id,
                    label: tag.name,
                    color: tag.color ?? tag.tagGroupColor,
                  }))}
                  page="tag"
                  onNavigate={onNavigate}
                />
...
export function ImageTile({
  ...
  selecting,
  bookmarkInitiallySaved,
}: ImageTileProps & { engagement?: EntityEngagement }) {
  const imageFit = useConfiguredImageFit();
  const likeCount = engagement?.likeCount ?? 0;
  const hasFavorite = engagement?.isFavorite === true;
  ...
  return (
    <div
      onClick={selecting ? (event) => onClick(toggleOptionsFromEvent(event)) : undefined}
      className={`entity-card group relative cursor-pointer overflow-hidden rounded-lg border bg-card text-left shadow-md shadow-black/20 flex flex-col h-full transition-colors ${selected ? "ring-2 ring-accent border-accent" : "border-border hover:border-accent/60"}`}
    >
      {!onPreview ? (
        <RouteCardLinkOverlay
          route={{ page: "image", id: image.id }}
          onClick={detailsClick}
          label={`Open image ${displayTitle}`}
          disabled={selecting}
          selectionSafeZone={selected !== undefined || selecting}
        />
      ) : null}
      <div
        className="card-media aspect-square overflow-hidden bg-surface relative"
        onClick={selecting ? undefined : () => previewClick()}
      >
        <EntityMedia
          entityType="image"
          entityId={image.id}
          ...
          className="h-full w-full"
          renderDefault={() => (
            <CoverImage src={imageUrl} alt={displayTitle} className="h-full w-full" loading="lazy" />
          )}
        />
        <RatingBanner rating={engagement?.rating} />
        {(selected !== undefined || selecting) && (
      ...
      <div className="card-body border-t border-border/50 p-2 flex-1 flex flex-col gap-1">
        {!selecting && onPreview ? (
          <a
            {...createRouteLinkProps<HTMLAnchorElement>({ page: "image", id: image.id }, detailsClick)}
            className="relative z-10 card-title font-semibold text-foreground line-clamp-2 group-hover:text-accent"
          >
            {displayTitle}
          </a>
        ) : (
          <p className="card-title font-semibold text-foreground line-clamp-2 group-hover:text-accent">
            {displayTitle}
          </p>
        )}
      </div>
        ...
        <>
          ...
          <div className="relative z-10 flex flex-wrap items-center justify-center gap-1 px-2 py-1.5 rounded-b card-popovers min-h-[28px]">
            ...
            {imageGroups.length > 0 ? (
              ...
              >
                <EntityLinkList
                  items={imageGroups.map((group) => ({ id: group.id, label: group.name }))}
                  page="group"
                  onNavigate={onNavigate}
                />
              </PopoverButton>
...
export function GalleryTile({
  ...
  selecting,
  bookmarkInitiallySaved,
}: GalleryTileProps & { engagement?: EntityEngagement }) {
...
function getGroupItemPreviewTitle(item: GroupItem, kind: GroupPreviewKind) {
  const explicitTitle =
    item.title?.trim() || item.videoTitle?.trim() || item.imageTitle?.trim() || item.childGroupName?.trim();
...
function GroupItemPreviewMedia({ item, kind }: { item: GroupItem; kind: GroupPreviewKind }) {
  ...
  if (kind === "video" && previewId) {
    return (
      <img
        src={videos.screenshotUrl(previewId, item.updatedAt)}
        alt=""
        className="h-9 w-14 flex-shrink-0 rounded bg-surface object-cover"
        loading="lazy"
        onError={(event) => {
  ...
  if (kind === "image" && previewId) {
    return (
      <CoverImage
        src={images.thumbnailUrl(previewId)}
        alt=""
        className="h-10 w-10 flex-shrink-0 rounded bg-surface"
        loading="lazy"
        onError={(event) => {
          (event.currentTarget as HTMLImageElement).style.display = "none";
        }}
...
function GroupItemPreviewRow({
  item,
  kind,
  onNavigate,
}: {
  item: GroupItem;
  kind: GroupPreviewKind;
  onNavigate?: (route: any) => void;
}) {
...
export function GroupItemsPopoverContent({
  groupId,
  kind,
  totalCount,
  onNavigate,
  ...
  if (matchingItems.length === 0)
    return <p className="px-1 text-[11px] text-muted">{GROUP_PREVIEW_EMPTY_LABELS[kind]}</p>;
...
export function GroupTile({
  ...
  isDragging,
  isOver,
}: GroupTileProps & { engagement?: EntityEngagement }) {
  const imageFit = useConfiguredImageFit();
  const previewCountItems: Array<{
    key: string;
    kind: GroupPreviewKind;
    title: string;
    count: number;
    ...
    <EntityTileFrame
      ...
      media={
        <>
          ...
          {group.kind === "dynamic" ? (
            <span className="absolute bottom-1 left-1 rounded bg-accent/90 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white">
              Dynamic
            </span>
          ) : null}
        </>
      ...
      body={
        <>
          ...
          {group.date || group.studioName ? (
            <p className="truncate text-xs text-secondary">{group.date || group.studioName}</p>
          ) : null}
        </>
      }
      footer={
          ...
          <>
            {group.videoCount > 0 ? (
              group.kind === "dynamic" ? (
                <PopoverButton
                  icon={<Film className="w-3.5 h-3.5" />}
                  count={group.videoCount}
                  title="Videos"
                  wide
                  preferBelow
                >
                  <GroupItemsPopoverContent
                    groupId={group.id}
                    kind="video"
                    totalCount={group.videoCount}
                    onNavigate={onNavigate}
                  />
                </PopoverButton>
              ) : (
            ...
            {countItems.map((item) => (
              <CountPill key={item.key} icon={item.icon} count={item.count} title={item.title} />
            ))}
            {hasUncategorizedItems ? (
              <CountPill icon={<Layers className="w-3.5 h-3.5" />} count={group.itemCount ?? 0} title="Items" />
            ) : null}
            {(group.tags?.length ?? 0) > 0 ? (
...
function CountPill({ icon, count, title }: { icon: ReactNode; count: number; title: string }) {
  return (
    <span
      className="inline-flex items-center gap-1 rounded px-1.5 py-1 text-xs text-muted"
      title={`${title}: ${count}`}
    >
      {icon}
      <span>{count}</span>
...
export function AudioTile({ audio, engagement, selected, onSelect, selecting, onClick, onNavigate }: AudioTileProps) {
  ...
  return (
    <article
      ...
      className={`entity-card group relative flex h-full cursor-pointer flex-col overflow-hidden rounded-lg border bg-card text-left transition-colors ${selected ? "border-accent ring-2 ring-accent" : "border-border hover:border-accent/60"}`}
    >
      <RouteCardLinkOverlay
        route={{ page: "audio", id: audio.id }}
        onClick={onClick}
        label={`Open ${title}`}
        disabled={selecting}
        selectionSafeZone={selected !== undefined || selecting}
      ...
      <div className="card-media relative flex aspect-video items-center justify-center overflow-hidden bg-surface">
        ...
        {audio.hasVideoFiles ? (
          <span className="absolute bottom-1 left-1 z-[5] inline-flex items-center gap-1 rounded bg-black/70 px-1.5 py-0.5 text-[10px] font-medium text-white">
            <MonitorPlay className="h-3 w-3" />
            Video
          </span>
        ) : null}
        {duration ? (
          <span className="absolute bottom-1 right-1 z-[5] rounded bg-black/70 px-1.5 py-0.5 text-xs text-white">
            {duration}
          </span>
        ) : null}
        <RatingBanner rating={engagement?.rating} />
      ...
      <div className="card-body flex min-h-0 flex-1 flex-col gap-1.5 border-t border-border/50 px-2.5 pb-2 pt-2">
        <div>
          <h2
            className="card-title line-clamp-2 font-semibold leading-snug text-foreground transition-colors group-hover:text-accent"
            title={title}
          >
            {title}
          </h2>
        ...
        <div data-audio-preview-ignore onMouseEnter={stopPreview}>
          <MediaCardPerformerBadges performerItems={audio.performers} onNavigate={onNavigate} />
        </div>
...
export function TextTile({ text, engagement, selected, onSelect, selecting, onClick, onNavigate }: TextTileProps) {
  ...
  const preview =
    primaryFile?.excerptText?.trim() ||
    text.details?.trim() ||
    "Open the document to read the extracted content and file details.";
    ...
    >
      ...
      <div className="card-media relative flex aspect-video items-center justify-center overflow-hidden bg-surface">
        <EntityMedia
          ...
          renderDefault={() =>
            text.imagePath ? (
              <CoverImage src={text.imagePath} alt={title} className="h-full w-full" loading="lazy" />
            ) : (
        ...
        {text.maxWordCount ? (
          <span className="absolute bottom-1 right-1 z-[5] rounded bg-black/70 px-1.5 py-0.5 text-xs text-white">
            {Intl.NumberFormat().format(text.maxWordCount)} words
          </span>
        ) : null}
        <RatingBanner rating={engagement?.rating} />
      </div>
    ...
    <EntityTileFrame
      ...
      media={
        <>
          <RatingBanner rating={engagement?.rating} />
          {favorite ? (
            <Heart className="absolute right-2 top-2 z-10 h-4 w-4 fill-red-500 text-red-500 drop-shadow" />
          ) : null}
          <EntityMedia
            entityType="tag"
            entityId={tag.id}
      ...
      body={
        <>
          <h3 className="card-title truncate text-sm font-semibold text-foreground group-hover:text-accent">
            {tag.name}
          </h3>
          {tag.tagGroupName ? (
            <div className="inline-flex max-w-full items-center gap-1.5 rounded-full border border-border bg-surface px-2 py-0.5 text-[10px] text-secondary">
              <span
                className="h-2 w-2 rounded-full border border-border"
                style={{ backgroundColor: tag.tagGroupColor ?? "transparent" }}
              />
      ...
      footer={
        hasFooter ? (
          <>
            {tag.videoCount != null && tag.videoCount > 0 ? (
              <PopoverButton icon={<Film className="w-3 h-3" />} count={tag.videoCount} title="Videos" wide preferBelow>
                <VideosPopoverContent filter={{ tagIds: String(tag.id) }} />
              </PopoverButton>
            ) : null}
            ...
            {tag.audioCount != null && tag.audioCount > 0 ? (
              <PopoverButton
                icon={<Headphones className="w-3.5 h-3.5" />}
                count={tag.audioCount}
                title="Audios"
                wide
                preferBelow
              >
                <AudiosPopoverContent filter={{ tagsCriterion: { modifier: "INCLUDES", value: [tag.id] } }} />
              </PopoverButton>
            ) : null}
            {tag.textCount != null && tag.textCount > 0 ? (
              <PopoverButton
                icon={<FileText className="w-3.5 h-3.5" />}
                count={tag.textCount}
                title="Texts"
                ...
                preferBelow
              >
                <TextsPopoverContent filter={{ tagsCriterion: { modifier: "INCLUDES", value: [tag.id] } }} />
              </PopoverButton>
            ) : null}
            {tag.galleryCount != null && tag.galleryCount > 0 ? (
              <PopoverButton
                icon={<FolderOpen className="w-3 h-3" />}
                count={tag.galleryCount}
                title="Galleries"
                wide
                preferBelow
              >
      ...
      media={
        <>
          ...
          <div className="absolute inset-x-0 bottom-0 flex flex-wrap gap-1 bg-gradient-to-t from-black/80 via-black/35 to-transparent p-2.5">
            {face.performerId ? (
              <span className="inline-flex items-center gap-1 rounded-full bg-black/65 px-2 py-0.5 text-[11px] text-white">
        ...
        <>
          ...
          <div className="truncate text-xs text-secondary">
            {face.performerId && face.performerName
              ? `Updated ${formatDate(face.updatedAt)}`
              : face.performerName || `Updated ${formatDate(face.updatedAt)}`}
          </div>
        </>
      }
      footer={
      ...
      {items.map((item) => (
        <div key={item.appearanceId} className="flex items-center gap-2 rounded px-1 py-0.5 hover:bg-card">
          <img
            src={item.thumbnailUrl}
            alt=""
            className="h-7 w-12 flex-shrink-0 rounded bg-surface object-cover"
            loading="lazy"
            onError={(event) => {
              (event.currentTarget as HTMLImageElement).style.display = "none";
            }}
      ...
      body={
        <>
          <div className="flex flex-wrap items-center gap-1.5 text-[11px] text-secondary">
            ...
            {appearance.hostType === "video" ? (
              <span>{formatFaceAppearanceTimeRange(appearance)}</span>
            ) : (
              <span>Image appearance</span>
            )}
          </div>
          <h3 className="card-title truncate text-sm font-semibold text-foreground group-hover:text-accent">
            {hostLabel}
          ...
          {appearance.topConfidence != null ? (
            <div className="text-xs text-secondary">{Math.round(appearance.topConfidence * 100)}% confidence</div>
          ) : null}
        </>
      }
      footer={
        <>
          ...
          <CountPill
            ...
            count={appearance.retainedSpatialSampleCount}
            title="Samples"
          />
...
export function SegmentTile({
  ...
  return (
    <article
      onClick={selecting ? (event) => onClick(toggleOptionsFromEvent(event)) : undefined}
      className={`entity-card video-card group relative overflow-hidden rounded border bg-card transition-all ${selected ? "border-accent ring-2 ring-accent" : "border-border hover:border-accent/60"}`}
    >
      <RouteCardLinkOverlay
        route={cardRoute}
        onClick={onClick}
        label={label ?? `Open segment ${title}`}
        disabled={selecting}
        selectionSafeZone={selected !== undefined || selecting}
      />
      <div className="card-media relative aspect-video w-full overflow-hidden bg-surface/70">
        ...
        {segment.hostType === "video" ? (
          <SegmentHoverPreview
            ...
            segmentId={typeof segment.id === "number" ? segment.id : undefined}
        ...
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 via-black/35 to-transparent p-3 text-white">
          <div className="flex items-end justify-between gap-3">
            ...
            <div className="shrink-0 rounded bg-black/70 px-1.5 py-0.5 text-xs font-medium text-white/85">
              {eyebrow ?? rangeLabel}
            </div>
          </div>
        </div>
      ...
      <div className="card-body border-t border-border bg-card p-3">
        ...
        <div className="truncate text-xs text-secondary">
          {segment.hostTitle || `${segment.hostType} #${segment.hostId}`}
        </div>
      </div>
      ...
      {footer ? (
        <div className="relative z-10 border-t border-border px-3 py-2 text-xs text-secondary">{footer}</div>
      ) : null}
    </article>
  );
}
