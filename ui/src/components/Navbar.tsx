...
export function Navbar({ currentPage, navigate }: NavbarProps) {
  ...
  const extensionNavItems = routes
    ...
    .map((r) => ({
      page: r.navItem!.page,
      label: r.navItem!.label,
      icon: r.navItem!.icon,
      order: r.navItem!.order ?? 99,
    }))
  ...
  >();
  for (const item of BUILT_IN_NAVIGATION_ITEMS.filter((navItem) => canShowNavPage(navItem.page, hasPermission, user)))
    allItemsMap.set(item.page, item);
  ...
  return (
    <nav
      className="cove-navbar bg-nav sticky top-0 z-50 shadow-lg shadow-black/30"
      role="navigation"
        ...
        <div className="flex min-w-0 items-center h-12">
          ...
          >
            {customLogo ? (
              <img
                src={customLogo}
                alt={config?.ui.title?.trim() || "Cove"}
                className="h-8 w-auto max-w-[200px] object-contain"
              />
            ) : (
              <svg
                viewBox="0 0 347.11 91.99"
            ...
            {allNavItems.map(({ page, label, icon: Icon }) => (
              <a
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded text-sm font-medium whitespace-nowrap transition-colors cursor-pointer ${
                  activePage === page ? "text-accent" : "text-secondary hover:text-foreground"
                }`}
              >
