...
export function FilterExpressionEditor({
  ...
  const legalDestinations = useCallback(
    (sourcePath: number[]) => {
      ...
      return destinations.filter((destination) => {
        ...
        if (
          destination.relatedScopeMatchMode === "distinct" &&
          destination.childCount >= MAX_DISTINCT_RELATED_CONDITIONS
        )
          return false;
    ...
    (sourcePath: number[], destinationPath: number[]) => {
      ...
      window.setTimeout(
        () =>
          window.setTimeout(() => {
            document
              .querySelector<HTMLElement>(`[data-expression-move-handle="${moved.insertedPath?.join(".")}"]`)
              ?.focus();
          }, 0),
        0,
      );
  ...
  const handleMoveKeyDown = useCallback(
    (sourcePath: number[], event: ReactKeyboardEvent<HTMLElement>) => {
      ...
      if ((event.key === " " || event.key === "Enter") && !active) {
        ...
        setMoveAnnouncement(
          `Condition picked up. Destination: ${legal[0].label}. Use Up and Down to choose a group, Enter to move, or Escape to cancel.`,
        );
        return;
      }
      if (!active) return;
  ...
  const activeKeyboardDestination = keyboardMove
    ? legalDestinations(keyboardMove.sourcePath)[keyboardMove.destinationIndex]
    : undefined;
  return (
...
function ExpressionGroupEditor({
  ...
  const relatedScopeCriterion = group.relatedScope
    ? criteria.find(
        (criterion) => criterion.type === "related" && criterion.filterKey === group.relatedScope?.filterKey,
      )
    : undefined;
  const relatedItemName =
    relatedScopeCriterion?.entityType === "galleries"
      ? "gallery"
      : (relatedScopeCriterion?.entityType?.replace(/s$/, "") ?? "item");
  const relatedItemArticle = /^[aeiou]/i.test(relatedItemName) ? "an" : "a";
  const distinctScopeAtLimit =
          ...
          () =>
            document
              .querySelector<HTMLButtonElement>(
                `[data-expression-operator-picker="${groupPathKey}"] button[aria-pressed="true"]`,
              )
              ?.focus(),
          0,
        ),
  ...
  const removeChild = (index: number) => {
    ...
    const nextFocusIndex =
      nextDisplayedChildren[Math.min(removedDisplayIndex, nextDisplayedChildren.length - 1)]?.index;
    setSelected(new Set());
    setOpenMenu(null);
  ...
  const ungroupChild = (index: number) => {
    ...
    const nextChildren = [
      ...
      ...group.children.slice(index + 1),
    ];
    const insertedIndexes = new Set(child.group.children.map((_, childIndex) => index + childIndex));
    const nextFocusIndex =
      sortFilterExpressionChildrenForDisplay(nextChildren, operator).find((entry) => insertedIndexes.has(entry.index))
        ?.index ?? index;
    onChange({ ...group, children: nextChildren });
  ...
  const groupSelected = (nextMode: "AND" | "OR" | "JUST_ONE" | "NONE") => {
    ...
    const children: FilterExpression<Record<string, unknown>>["children"] = group.children.flatMap((child, index) =>
      index === first
        ? [
            {
              group: (nextMode === "NONE"
                ...
                : {
                    ...
                    ...(scopedCriterion
                      ? { relatedScope: { filterKey: scopedCriterion.filterKey, matchMode: "reuse" as const } }
                      : {}),
                  }) as EditableFilterExpression,
            },
          ]
  ...
  const closeMenuAndRestoreFocus = () => {
    const trigger =
      openMenu === "group"
        ? groupMenuButtonRef.current
        : openMenu === null
          ? null
          : conditionMenuButtonRefs.current.get(openMenu);
    setOpenMenu(null);
    window.setTimeout(() => trigger?.focus(), 0);
  };
  const isActiveMoveDestination = Boolean(
    activeKeyboardDestination && expressionPathsEqual(activeKeyboardDestination.path, groupPath),
  );
  const isAvailableDropDestination = Boolean(
    draggedPath &&
  ...
  return (
    <section
      ...
      aria-label={
        relatedScopeCriterion
          ? `${relatedScopeCriterion.label} ${presentation.label} group`
          : `${presentation.label} group`
      }
    ...
    >
      <span
        aria-hidden="true"
        className={`absolute bottom-2 left-1.5 top-2 w-0.5 rounded-full ${presentation.railClassName}`}
      />
      <div className="flex min-h-10 flex-wrap items-center gap-2">
        {relatedScopeCriterion ? (
          <span className="rounded-md bg-accent/10 px-2 py-1 text-sm font-semibold text-accent">
            {relatedScopeCriterion.label}
          </span>
        ) : null}
        {mode === "NOT" ? (
          <span className={`rounded-md px-2 py-1 text-sm font-semibold ${presentation.labelClassName}`}>
            {operatorText}
          </span>
        ) : (
          <>
            <button
              ...
              aria-label={`${operatorPickerOpen ? "Close" : "Change"} ${operatorText} operator`}
              aria-expanded={operatorPickerOpen}
              title={`Change ${presentation.label} group operator`}
              onClick={() => {
                setOpenMenu(null);
                setMoveMenuIndex(null);
                setOperatorPickerOpen((current) => !current);
              }}
              className={`rounded-md px-2 py-1 text-sm font-semibold ${presentation.labelClassName}`}
            >
              {operatorText}
            </button>
            {operatorPickerOpen ? (
              <div
                data-expression-operator-picker={groupPathKey}
                className="inline-flex max-w-full rounded-lg bg-card p-1"
                role="group"
              ...
              >
                <button
                  ...
                  className={`min-h-8 rounded-md px-2.5 text-sm ${mode === "AND" ? FILTER_EXPRESSION_OPERATOR_PRESENTATION.AND.selectedClassName : "text-secondary hover:text-foreground"}`}
                >
                  All
                </button>
                <button
                  ...
                  className={`min-h-8 rounded-md px-2.5 text-sm ${mode === "OR" ? FILTER_EXPRESSION_OPERATOR_PRESENTATION.OR.selectedClassName : "text-secondary hover:text-foreground"}`}
                >
                  Any
                </button>
                <button
                  type="button"
                  aria-label="Just One"
                  aria-pressed={mode === "JUST_ONE"}
                  onClick={() => setOperator("JUST_ONE")}
                  className={`min-h-8 rounded-md px-2.5 text-sm ${mode === "JUST_ONE" ? FILTER_EXPRESSION_OPERATOR_PRESENTATION.JUST_ONE.selectedClassName : "text-secondary hover:text-foreground"}`}
                >
                  Just One
                </button>
                <button
                  ...
                  className={`min-h-8 rounded-md px-2.5 text-sm ${mode === "NONE" ? FILTER_EXPRESSION_OPERATOR_PRESENTATION.NONE.selectedClassName : "text-secondary hover:text-foreground"}`}
                >
                  None
                </button>
              </div>
        ...
        <div data-expression-group-actions className="relative ml-auto flex items-center gap-1">
          {!root ? (
            <button
              type="button"
              aria-label={`${collapsed ? "Expand" : "Collapse"} ${presentation.label} group`}
              ...
              className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-muted hover:bg-card hover:text-foreground"
            >
          ...
          {mode !== "NOT" ? (
            <button
              ref={addConditionRef}
              type="button"
              aria-label={relatedScopeCriterion ? `Add ${relatedItemName} condition` : "Add condition"}
              title={
                distinctScopeAtLimit
                  ? `Distinct assignment supports up to ${MAX_DISTINCT_RELATED_CONDITIONS} conditions`
                  : "Add condition"
              }
              disabled={conditionCount >= MAX_FILTER_EXPRESSION_CONDITIONS || distinctScopeAtLimit}
              onClick={() => {
                setOperatorPickerOpen(false);
                setOpenMenu(null);
                setMoveMenuIndex(null);
                setCollapsed(false);
                onAddCondition(relatedScopeCriterion?.id, groupPath);
              }}
              data-expression-return-focus={`add-${groupPath.join(".")}`}
              className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-secondary hover:bg-card hover:text-foreground disabled:opacity-40"
            >
          ...
          {hasGroupActions ? (
            <>
              <button
                ...
                type="button"
                data-expression-group-control={operator === "NOT" ? groupPathKey : undefined}
                aria-label={`More actions for ${root ? "root " : ""}group`}
              ...
              {openMenu === "group" ? (
                <div
                  ...
                  className="absolute right-0 top-full z-20 mt-1 min-w-44 rounded-lg border border-border bg-surface p-1 shadow-xl"
                >
                  {mode !== "NOT" && group.children.length > 0 ? (
                    <button
                      type="button"
                      onClick={() => {
                        setOpenMenu(null);
                      ...
                      disabled={!canCreateNestedGroup}
                      className="block min-h-10 w-full rounded px-3 text-left text-sm hover:bg-card disabled:opacity-40"
                    >
                      Create subgroup
                    </button>
                  ) : null}
                  {!root ? (
                    <>
                      <button
                        ...
                        disabled={!ungroupChildFromParent || (parentOperator === "NOT" && group.children.length !== 1)}
                        className="block min-h-10 w-full rounded px-3 text-left text-sm hover:bg-card disabled:opacity-40"
                      >
                        Dissolve group
                      </button>
                      <button
                        ...
                        disabled={!removeGroupFromParent || parentOperator === "NOT"}
                        className="block min-h-10 w-full rounded px-3 text-left text-sm text-red-300 hover:bg-red-500/10 disabled:opacity-40"
                      >
                        Remove group
                      </button>
                    </>
      ...
      {!collapsed && mode === "AND" && relatedScopeCriterion ? (
        <div
          className="flex flex-wrap items-center gap-2 px-2"
          role="group"
          aria-label={`How ${relatedScopeCriterion.label.toLowerCase()} conditions choose matches`}
        >
          <span className="text-xs font-medium text-muted">Match assignment</span>
          <button
            type="button"
            aria-pressed={group.relatedScope?.matchMode !== "distinct"}
            onClick={() =>
              onChange({ ...group, relatedScope: { filterKey: relatedScopeCriterion.filterKey, matchMode: "reuse" } })
            }
            className={`min-h-8 rounded-md px-2.5 text-sm ${group.relatedScope?.matchMode !== "distinct" ? "bg-accent/15 text-accent" : "text-secondary hover:bg-card hover:text-foreground"}`}
          >
            May reuse {relatedItemArticle} {relatedItemName}
          </button>
          <button
            type="button"
            aria-pressed={group.relatedScope?.matchMode === "distinct"}
            disabled={group.children.length > MAX_DISTINCT_RELATED_CONDITIONS}
            onClick={() =>
              onChange({
                ...group,
                relatedScope: { filterKey: relatedScopeCriterion.filterKey, matchMode: "distinct" },
              })
            }
            className={`min-h-8 rounded-md px-2.5 text-sm ${group.relatedScope?.matchMode === "distinct" ? "bg-accent/15 text-accent" : "text-secondary hover:bg-card hover:text-foreground"} disabled:opacity-40`}
          >
            Use a different {relatedItemName} for each
          </button>
      ...
      {!collapsed ? (
        <div className="space-y-2" data-testid="expression-group-children">
          {displayedChildren.map(({ child, index }, displayIndex) => {
            ...
            const isLastDisplayedCondition = Boolean(
              child.filter && displayedConditionIndex === displayedConditionEntries.length - 1,
            );
            ...
            return (
              <div key={index} className="space-y-2">
                {isReorderingHere && child.filter ? (
                  <div
                    ...
                    onDrop={(event) => {
                      event.preventDefault();
                      event.stopPropagation();
                      if (sourceIndex !== undefined) reorderChild(sourceIndex, index);
                    }}
                  ...
                  >
                    <span className="h-0.5 flex-1 bg-accent/60" />
                    <span>Drop here</span>
                    <span className="h-0.5 flex-1 bg-accent/60" />
                  </div>
                ) : null}
                {groupingMode ? (
                  <button
                    ref={(element) => {
                      if (element) childFocusRefs.current.set(index, element);
                      else childFocusRefs.current.delete(index);
                    }}
                    type="button"
                  ...
                  >
                    <span
                      aria-hidden="true"
                      className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${selected.has(index) ? "border-accent bg-accent text-white" : "border-muted"}`}
                    >
                      {selected.has(index) ? "✓" : ""}
                    </span>
                    <span>
                      {child.group
                        ? `${FILTER_EXPRESSION_OPERATOR_PRESENTATION[normalizeFilterExpressionOperator(child.group.operator)].label} group`
                  ...
                  <div
                    ...
                    onFocus={(event) => {
                      if (event.target === event.currentTarget)
                        event.currentTarget
                          .querySelector<HTMLButtonElement>("button:not([data-expression-move-handle])")
                          ?.focus();
                    }}
                    role="group"
                  ...
                  >
                    <button
                      ...
                      aria-label={
                        isKeyboardMoving && activeKeyboardDestination
                          ? `Drop condition in ${activeKeyboardDestination.label}`
                          : `Move condition ${displayPosition}`
                      }
                    ...
                    {isDraftOnly ? (
                      <button
                        type="button"
                        onClick={() => onEditCondition(childPath)}
                        className="flex min-h-7 min-w-0 items-center overflow-hidden rounded-md border border-border bg-surface/70 px-2 text-left text-xs text-muted"
                        aria-label={`Edit condition ${displayPosition}: ${describeCondition(child.filter ?? {})}`}
                      >
                        {describeCondition(child.filter ?? {})}
                      </button>
                    ) : (
                    ...
                    <div className="relative flex items-center pr-1">
                      <button
                        ref={(element) => {
                          if (element) conditionMenuButtonRefs.current.set(index, element);
                          else conditionMenuButtonRefs.current.delete(index);
                        }}
                        type="button"
                        aria-label={`More actions for condition ${displayPosition}`}
                        ...
                        className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-muted opacity-100 hover:bg-card hover:text-foreground md:opacity-0 md:group-hover/condition:opacity-100 md:group-focus-within/condition:opacity-100"
                      >
                        <MoreHorizontal className="h-4 w-4" />
                      </button>
                      {openMenu === index ? (
                        <div
                          ...
                          aria-label={
                            moveMenuIndex === index
                              ? `Move condition ${displayPosition}`
                              : `Condition ${displayPosition} actions`
                          }
                          className="absolute right-0 top-full z-20 mt-1 max-h-72 min-w-56 overflow-y-auto rounded-lg border border-border bg-surface p-1 shadow-xl"
                        >
                          {moveMenuIndex === index ? (
                            <>
                              <button
                                type="button"
                                onClick={() => setMoveMenuIndex(null)}
                                className="block min-h-10 w-full rounded px-3 text-left text-sm text-secondary hover:bg-card"
                              >
                                Back
                              </button>
                              {conditionDestinations.map((destination) => (
                                <button
                                  key={destination.path.join(".") || "root"}
                                  type="button"
                                  onClick={() => {
                                    setOpenMenu(null);
                                    setMoveMenuIndex(null);
                                    onMoveCondition(childPath, destination.path);
                                  }}
                                  style={{ paddingLeft: `${0.75 + destination.depth * 0.75}rem` }}
                                  className="block min-h-10 w-full rounded pr-3 text-left text-sm hover:bg-card"
                                >
                                  {destination.label}
                                </button>
                              ))}
                            ...
                            <>
                              <button
                                ...
                                onClick={() => {
                                  setOpenMenu(null);
                                  onEditCondition([...groupPath, index]);
                                }}
                                className="block min-h-10 w-full rounded px-3 text-left text-sm hover:bg-card"
                              >
                                Edit
                              </button>
                              <button
                                ...
                                onClick={() => {
                                  setOpenMenu(null);
                                  if (previousConditionIndex !== undefined) reorderChild(index, previousConditionIndex);
                                }}
                                disabled={previousConditionIndex === undefined}
                                className="block min-h-10 w-full rounded px-3 text-left text-sm hover:bg-card disabled:opacity-40"
                              >
                                Move earlier
                              </button>
                              <button
                                ...
                                onClick={() => {
                                  setOpenMenu(null);
                                  if (nextConditionIndex !== undefined) reorderChild(index, nextConditionIndex + 1);
                                }}
                                disabled={nextConditionIndex === undefined}
                                className="block min-h-10 w-full rounded px-3 text-left text-sm hover:bg-card disabled:opacity-40"
                              >
                                Move later
                              </button>
                              <button
                                type="button"
                                onClick={() => setMoveMenuIndex(index)}
                                disabled={conditionDestinations.length === 0}
                                className="block min-h-10 w-full rounded px-3 text-left text-sm hover:bg-card disabled:opacity-40"
                              >
                                Move to…
                              </button>
                              <button
                                ...
                                disabled={group.operator === "NOT" && group.children.length === 1}
                                className="block min-h-10 w-full rounded px-3 text-left text-sm text-red-300 hover:bg-red-500/10 disabled:opacity-40"
                              >
                                Remove
                              </button>
                ...
                {isReorderingHere && isLastDisplayedCondition ? (
                  <div
                    ...
                    onDrop={(event) => {
                      event.preventDefault();
                      event.stopPropagation();
                      if (sourceIndex !== undefined) reorderChild(sourceIndex, index + 1);
                    }}
                    className="flex h-7 w-full items-center gap-2 rounded-md border border-dashed border-accent/60 px-2 text-xs text-accent hover:bg-accent/10 focus-visible:ring-2 focus-visible:ring-accent"
                  >
                    <span className="h-0.5 flex-1 bg-accent/60" />
          ...
          {group.children.length === 0 ? (
            <p className="px-3 py-5 text-center text-sm text-muted">No conditions in this group.</p>
          ) : null}
        </div>
      ) : null}
      {!collapsed ? (
        <div className="flex flex-wrap items-center gap-2 pt-1">
            ...
            <div className="w-full space-y-2">
              <div className="flex min-h-10 items-center justify-between gap-3">
                ...
                <button
                  ...
                  className="min-h-10 rounded-lg px-3 text-sm text-secondary hover:bg-card hover:text-foreground"
                >
                  Cancel grouping
                </button>
              </div>
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                <button
                  type="button"
                  aria-label="Group selected as All"
                  disabled={selected.size < 2}
                  onClick={() => groupSelected("AND")}
                  className="min-h-10 rounded-lg border border-border px-2 text-sm text-secondary hover:bg-card hover:text-foreground disabled:opacity-40"
                >
                  All
                </button>
                <button
                  type="button"
                  aria-label="Group selected as Any"
                  disabled={selected.size < 2}
                ...
                <button
                  type="button"
                  aria-label="Group selected as Just One"
                  disabled={selected.size < 2}
                  onClick={() => groupSelected("JUST_ONE")}
                  className="min-h-10 rounded-lg border border-border px-2 text-sm text-secondary hover:bg-card hover:text-foreground disabled:opacity-40"
                >
                  Just One
                </button>
                <button
                  type="button"
                  aria-label="Group selected as None"
                  disabled={selected.size < 1}
                  onClick={() => groupSelected("NONE")}
                  className="min-h-10 rounded-lg border border-border px-2 text-sm text-secondary hover:bg-card hover:text-foreground disabled:opacity-40"
                >
                  None
                </button>
          ...
          {conditionCount >= MAX_FILTER_EXPRESSION_CONDITIONS ? (
            <span className="text-xs text-muted">
              Maximum of {MAX_FILTER_EXPRESSION_CONDITIONS} conditions reached.
            </span>
          ) : null}
          <span className="sr-only" role="status">
            {announcement}
