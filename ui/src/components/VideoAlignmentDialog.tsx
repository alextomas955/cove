...
export function VideoAlignmentDialog({
  videoId,
  targetFileId = 0,
  onClose,
  onApplied = () => {},
  ...
  useEffect(() => {
    ...
    videoAlignments
      .assess(videoId, targetFileId, abort.signal)
      .then((value) => {
        setAssessment(value);
        setBusy("");
      })
  ...
  async function approveAlignment(signal: AbortSignal) {
    const deletedClips = unresolved.filter(
      (dependency) => dependency.kind === "clip" && deletions.has(`clip:${dependency.id}`),
    ).length;
    if (
      deletedClips > 0 &&
      !window.confirm(
        `This will permanently delete ${deletedClips} clip video${deletedClips === 1 ? "" : "s"} and any content nested under them. Continue?`,
      )
    )
      return;
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/70 p-3">
      <div
        ...
        className="max-h-[95vh] w-full max-w-6xl overflow-y-auto rounded-xl border border-border bg-background p-5 text-foreground shadow-xl"
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 id="alignment-title" className="text-xl font-semibold">
              Set primary video file
            </h2>
            <p className="mt-1 max-w-3xl text-sm text-secondary">{dialogIntroduction}</p>
          </div>
          <button className={control} onClick={close}>
            Cancel
          </button>
        </div>
        {assessment && (assessment.equivalent || assessment.dependencyCount === 0) && (
          <div className="my-4 rounded border border-border bg-surface p-3 text-sm">
            <p>
              {assessment.sourceDuration == null
                ? `This video has no primary file. The selected file is ${seconds(assessment.targetDuration)} long.`
                : `Duration changes from ${seconds(assessment.sourceDuration)} to ${seconds(assessment.targetDuration)}.`}
            </p>
            <p>
              {assessment.dependencyCount === 0
                ? "No timed content is affected."
                : "Affected timed content will keep the same timestamps."}
            </p>
            {assessment.equivalent && (
              <p className="mt-1 text-green-400">
                The pHashes and durations match within tolerance, so timestamps can be kept.
              </p>
            )}
          </div>
        )}
        {assessment && (assessment.equivalent || assessment.dependencyCount === 0) && (
          <button
            disabled={!!busy}
            className={`${control} bg-accent text-white`}
            onClick={() => void run("Setting primary file…", (signal) => apply("direct", signal))}
          >
            Set as primary
          </button>
        )}
        {assessment && !assessment.equivalent && assessment.dependencyCount > 0 && !preview && (
          <div className="mt-5 space-y-3">
            <p className="text-xs font-medium uppercase tracking-wide text-secondary">
              {assessment.sourceFileId == null || !assessment.canAlign
                ? "Decision required"
                : "Step 1 of 2 · Choose an approach"}
            </p>
            <p className="text-sm text-secondary">
              {assessment.sourceDuration == null
                ? `The selected file is ${seconds(assessment.targetDuration)} long. There is no current primary duration to compare.`
                : `Duration changes from ${seconds(assessment.sourceDuration)} to ${seconds(assessment.targetDuration)}.`}
            </p>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <section className="flex flex-col rounded-lg border border-border bg-surface p-4">
                ...
                <p className="mt-2 flex-1 text-sm text-secondary">
                  Compare samples from positions across both files, confirm one consistent timeline offset, adjust the
                  related timestamps, and show side-by-side previews before anything changes.
                </p>
                {assessment.sourceFileId != null && assessment.canAlign ? (
                  <button
                    disabled={!!busy}
                    className="mt-4 rounded border border-accent bg-accent px-3 py-2 text-sm text-white hover:brightness-110 disabled:opacity-50"
                    onClick={() =>
                      void run("Sampling both files and checking the timeline offset…", async (signal) => {
                        const value = await videoAlignments.analyze(
                          videoId,
                          assessment.sourceFileId!,
                          assessment.targetFileId,
                          signal,
                        );
                        if (value.anchors.length < 2) throw new Error(value.message);
                        setAnchors(value.anchors);
                        setBusy("Preparing side-by-side review…");
                        const review = await videoAlignments.preview(
                          videoId,
                          {
                            sourceFileId: assessment.sourceFileId!,
                            targetFileId,
                            sourceDuration: assessment.sourceDuration!,
                            targetDuration: assessment.targetDuration,
                            anchors: value.anchors,
                          },
                          signal,
                        );
                        if (!Array.isArray(review.comparisons) || !review.comparisonCounts)
                          throw new Error(
                            "The alignment review is unavailable. Refresh after the Cove server finishes restarting, then try again.",
                          );
                        setPreview(review);
                      })
                    }
                  ...
                  <p className="mt-4 text-sm text-amber-300">
                    The current or selected file is unavailable, so Cove cannot find matching anchor points.
                  </p>
                )}
              </section>
              <section className="flex flex-col rounded-lg border border-border bg-surface p-4">
                ...
                <p className="mt-2 flex-1 text-sm text-secondary">
                  Permanently remove all affected timed content and set the selected file as primary.
                  {(assessment.dependencyCounts.clip ?? 0) > 0
                    ? " Clip videos and anything nested under them will also be deleted."
                    : ""}{" "}
                  This cannot be undone.
                </p>
                <button
                  disabled={!!busy}
                  className={`${control} mt-4 border-red-500/60 text-red-300 hover:bg-red-500/10`}
                  onClick={() => {
                    if (
                      window.confirm(
                        "Permanently delete all affected timed content, including clip videos and anything nested under them, then set this file as primary?",
                      )
                    )
                      void run("Deleting timed content and setting primary file…", (signal) => apply("delete", signal));
                  }}
                >
                  Remove all timed content
                </button>
              </section>
            </div>
          </div>
          ...
          <div className="mt-4 space-y-4">
            <div className="rounded border border-border p-3">
              <p className="mb-1 text-xs font-medium uppercase tracking-wide text-secondary">
                Step 2 of 2 · Review and approve
              </p>
              <h3 className="font-medium">Review alignment</h3>
              <p className="mt-1 text-sm text-secondary">
                Showing {comparisons.filter((item) => item.kind === "segment").length} of {comparisonCounts.segment}{" "}
                segments and {comparisons.filter((item) => item.kind === "clip").length} of {comparisonCounts.clip}{" "}
                clips at their mapped positions.
              </p>
              <p className="mt-1 text-xs text-secondary">
                The visible comparison plays both timelines together so you can confirm their timing.
              </p>
              {comparisons.length > 0 ? (
                <div className="mt-3 max-h-[55vh] space-y-4 overflow-y-auto pr-1">
                  {comparisons.map((item) => {
                    const dependency = preview.dependencies.find(
                      (candidate) => candidate.kind === item.kind && candidate.id === item.id,
                    );
                    const sourceRange = reviewPlaybackRange(
                      dependency?.startSec,
                      dependency?.endSec,
                      item.sourceSec,
                      assessment?.sourceDuration,
                    );
                    const targetRange = reviewPlaybackRange(
                      dependency?.mapped?.startSec,
                      dependency?.mapped?.endSec,
                      item.targetSec,
                      assessment?.targetDuration,
                    );
                    ...
                    return (
                      ...
                      >
                        <div className="mb-2 flex flex-wrap items-baseline justify-between gap-2">
                          <h4
                            aria-label={`${item.kind === "segment" ? "Segment" : "Clip"}: ${item.title || "Untitled"}`}
                            className="flex items-center gap-1.5 font-medium"
                          >
                            <ComparisonKindIcon kind={item.kind} />
                            {item.title || "Untitled"}
                          </h4>
                          <span className="text-xs text-secondary">
                            {seconds(item.sourceSec)} → {seconds(item.targetSec)}
                          </span>
                        </div>
                        <SynchronizedReviewPair
                          ...
                          source={{
                            ...
                            videoSrc: videos.streamUrl(videoId, assessment!.sourceFileId!),
                            ...sourceRange,
                          }}
                          target={{
                            label: "New primary",
                            thumbnail: item.targetThumbnail,
                            videoSrc: videos.streamUrl(videoId, targetFileId),
                            ...targetRange,
                          }}
                        />
                      </ComparisonRow>
                ...
                <p className="mt-3 text-sm text-secondary">
                  There are no mapped segments or clips to compare visually.
                </p>
              )}
            </div>
            {unresolved.length > 0 && (
              <div className="rounded border border-amber-500/50 p-3">
                <p className="text-sm">
                  Some timed content falls outside the matched sections. Select each item for permanent deletion before
                  approving.
                </p>
                <ul className="mt-2 max-h-56 overflow-y-auto text-sm">
                  {unresolved.map((dependency) => {
                    ...
                    const deletionLabel =
                      dependency.kind === "clip" ? "Delete clip video" : `Delete ${dependency.kind}`;
                    return (
                      <li key={key} className="flex items-center gap-2 border-t border-border py-2">
                        <span className="flex-1">
                          {dependency.kind}: {dependency.title || "Untitled"} · {seconds(dependency.startSec)} · no
                          counterpart found
                        </span>
                        <label className="flex items-center gap-1">
                          <input
                            type="checkbox"
                            checked={deletions.has(key)}
                            onChange={(event) =>
            ...
            <button
              ...
              onClick={() => void run("Applying alignment and setting primary file…", approveAlignment)}
            >
              Approve alignment and set primary
            </button>
          </div>
        )}
        {busy && (
          <div className="mt-4 flex items-center gap-3" role="status">
            <span>{busy}</span>
            <button className={control} onClick={() => operation.current?.abort()}>
              Cancel operation
            </button>
          </div>
        )}
...
function ComparisonKindIcon({ kind }: { kind: "segment" | "clip" }) {
  ...
  return (
    <span title={label} className="inline-flex text-secondary">
      {kind === "segment" ? (
...
function DependencyCountList({ counts }: { counts: Record<string, number> }) {
  ...
  return (
    <ul aria-label="Affected timed content" className="flex flex-wrap gap-2">
      {entries.map(({ kind, count, label }) => (
        <li
          key={kind}
          aria-label={`${count} ${label}`}
          className="inline-flex min-w-36 items-center gap-2 rounded border border-border bg-surface px-3 py-2 text-sm"
        >
...
function ComparisonRow({
  ...
  return (
    <div
      ...
      aria-label={label}
      aria-pressed={active}
      className={`rounded border bg-surface p-3 ${active ? "border-accent/60" : "border-border"}`}
      onClick={onActivate}
      onKeyDown={(event) => {
...
function SynchronizedReviewPair({
  active,
  source,
  target,
}: {
  active: boolean;
  source: Omit<ReviewFrameProps, "active">;
  target: Omit<ReviewFrameProps, "active">;
}) {
