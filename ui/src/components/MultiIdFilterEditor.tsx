...
export function MultiIdEditor({
  ...
  const [selectedValueFocusId, setSelectedValueFocusId] = useState<number | null>(
    () => includedIds[0] ?? excludedIds[0] ?? null,
  );
  ...
  const {
    ...
    queryFn: async () => {
      ...
      switch (entityType) {
        ...
        case "galleries":
          return (
            await galleriesApi.find({ q: trimmedSearchText || undefined, perPage: 50, sort: "title", direction: "asc" })
          ).items;
        case "videos":
          return (
            await videosApi.find({ q: trimmedSearchText || undefined, perPage: 50, sort: "title", direction: "asc" })
          ).items;
        case "audios":
          return (
            await audiosApi.find({ q: trimmedSearchText || undefined, perPage: 50, sort: "title", direction: "asc" })
          ).items;
        case "faces":
          return (await facesApi.list({ q: trimmedSearchText || undefined, merged: false, page: 1, perPage: 50 }))
            .items;
        default:
          return [];
      }
  ...
  const nameMap = useMemo(() => {
    ...
    if (entities)
      for (const e of entities as any[])
        map[String(e.id)] = e.name || e.title || getMultiIdEntityLabels(entityType).singular;
  ...
  const handleMatchModeKeyDown = (event: ReactKeyboardEvent<HTMLDivElement>) => {
    ...
    event.currentTarget.querySelector<HTMLButtonElement>(`button[data-match-modifier="${nextModifier}"]`)?.focus();
    ...
    <div className="flex min-h-full flex-1 flex-col gap-2">
      ...
      {isNullModifier ? (
        <div className="rounded border border-border/70 bg-input px-2 py-2 text-xs text-muted">
          This criterion will match entities with {modifier === "IS_NULL" ? "no" : "at least one"} linked {entityType}{" "}
          item.
        </div>
      ) : (
        <>
          ...
          <div className="relative">
            <input
              ...
              aria-activedescendant={
                activeResultIndex >= 0
                  ? `multi-id-result-${entityType}-${navigableEntities[activeResultIndex]?.id}`
                  : undefined
              }
          ...
          <p className="hidden text-xs text-muted md:block">
            Use ↑/↓ to choose, Enter to include, Shift+Enter to exclude. Ctrl/⌘+Enter applies.
          </p>
          <div
            ref={resultsRef}
            id={`multi-id-results-${entityType}`}
            role="listbox"
            aria-label={`${labels.plural} results`}
            tabIndex={-1}
            className="max-h-64 shrink-0 overflow-y-auto rounded-lg border border-border bg-input"
            aria-busy={isPlaceholderData || isFetching || undefined}
          >
            ...
            <div className="flex flex-wrap gap-1" role="group" aria-label={`Included ${labels.plural}`}>
              {includedIds.map((id) => {
                ...
                return (
                  <span
                    key={id}
                    className="inline-flex h-8 items-center overflow-hidden rounded-md border border-green-700 bg-green-900/50 pl-2 text-sm text-green-300 focus-within:ring-2 focus-within:ring-accent"
                  >
                    ...
                    <button
                      ...
                      tabIndex={
                        selectedValueFocusId === id || (selectedValueFocusId == null && id === selectedIds[0]) ? 0 : -1
                      }
            ...
            <div className="flex flex-wrap gap-1" role="group" aria-label={`Excluded ${labels.plural}`}>
              {excludedIds.map((id) => {
                ...
                return (
                  <span
                    key={id}
                    className="inline-flex h-8 items-center overflow-hidden rounded-md border border-red-700 bg-red-900/50 pl-2 text-sm text-red-300 focus-within:ring-2 focus-within:ring-accent"
                  >
                    <span className="max-w-56 truncate">{getSelectedName(id, entity)}</span>
                    <button
                      type="button"
          ...
          {selectedIds.length > 0 ? (
            <p id={selectedValueInstructionsId} className="text-xs text-muted">
              Selected {labels.plural}: use ←/→ to review; Delete removes.
            </p>
          ) : null}
