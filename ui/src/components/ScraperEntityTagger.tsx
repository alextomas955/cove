...
function buildDefaultApplyPlan(
  ...
  if (scrapedData.title && scrapedData.title !== currentData.title)
    replaceFields.push(entityType === "group" ? "name" : "title");
  ...
  if (entityType === "group" && scrapedData.director && scrapedData.director !== currentData.director)
    replaceFields.push("director");
  if (entityType === "group" && scrapedData.duration && scrapedData.duration !== currentData.duration)
    replaceFields.push("duration");
  if (entityType === "group" && scrapedData.rating && scrapedData.rating !== currentData.rating)
    replaceFields.push("rating");
  if (entityType === "group" && scrapedData.imageUrl && scrapedData.imageUrl !== currentData.imageUrl)
    replaceFields.push("image");
  if (
    (entityType === "image" || entityType === "gallery") &&
    scrapedData.creator &&
    scrapedData.creator !== currentData.creator
  )
  ...
  return {
    ...
    collectionModes: {
      ...
      aliases:
        entityType === "group" &&
        scrapedData.aliases.length > 0 &&
        !listsEqual(scrapedData.aliases, currentData.aliases)
          ? "merge"
          : "skip",
      ...
      performers:
        entityType !== "group" &&
        scrapedData.performers.length > 0 &&
        !listsEqual(scrapedData.performers, currentData.performers)
          ? "merge"
          : "skip",
    },
  };
  ...
  return {
    ...
    performerSelections:
      result.performerNames.length > 0
        ? buildRelationSelectionPayload(result.performerNames, performerActions)
        : undefined,
  };
}

export function ScraperEntityTagger<T extends ScraperEntityItem>({
  entityType,
  label,
  items,
  selectedIds,
  ...
  const searchItem = useCallback(
    async (item: T) => {
      ...
      try {
        ...
        const results = parseAttemptResults(attempt).map((result, index) =>
          mapScraperResult(attempt, result, index, selectedScraper, entityType),
        );
    ...
    return (
      <div className="px-4 py-12 text-center">
        ...
        <p className="text-muted text-sm mt-1">
          Install or enable a scraper that supports {label.toLowerCase()} metadata.
        </p>
      </div>
    );
  }
        ...
        <TaggerSettingsPanel denylist={denylist} onDenylistChange={updateDenylist}>
          <label className="flex items-center gap-2 text-xs text-foreground">
            <input
              ...
              onChange={(event) => updatePreferences({ createMissingTags: event.target.checked })}
          ...
          {entityType !== "group" && (
            <label className="flex items-center gap-2 text-xs text-foreground">
              <input
                ...
                onChange={(event) => updatePreferences({ createMissingPerformers: event.target.checked })}
                className="rounded border-border"
              />
          ...
          <label className="flex items-center gap-2 text-xs text-foreground">
            <input
              ...
              onChange={(event) => updatePreferences({ createMissingStudio: event.target.checked })}
              className="rounded border-border"
            />
...
function ScraperEntityTaggerRow({
  ...
  const applyPlan = useMemo(
    () => buildDefaultApplyPlan(entityType, item, selectedResult),
    [entityType, item, selectedResult],
  );
  ...
  const preview = (
    <>
      ...
      <p
        className={`mt-0.5 truncate text-[11px] font-medium leading-snug ${itemLinkProps ? "text-accent group-hover/entity:underline" : "text-foreground"}`}
      >
        {title}
      </p>
      <p className="truncate text-[10px] text-muted">
        {item.studioName || item.files?.[0]?.basename || item.date || ""}
      </p>
    </>
  );
    ...
    <div className={`px-3 py-2 ${selected ? "bg-accent/5" : ""}`}>
      <div className="flex gap-3">
        {onSelect && (
          <button
            type="button"
            onClick={(event) => onSelect(item.id, toggleOptionsFromEvent(event))}
            ...
            <div className="space-y-1">
              {state.results.map((result, index) => (
                <ScraperResultRow
                  ...
                  onReplaceFieldChange={(field, shouldReplace) =>
...
function ScraperResultRow({
  ...
  const scalarRows = [
    {
      ...
      label: entityType === "group" ? "Name" : "Title",
    ...
    ...(entityType === "group"
      ? [
          ...
          { key: "rating", label: "Rating", current: currentData.rating, scraped: result.rating },
          {
            key: "image",
            label: "Front image",
            current: currentData.imageUrl ? "Current front image" : undefined,
            scraped: result.imageUrl ? "Scraped front image" : undefined,
          },
        ]
      : []),
    ...(entityType === "image" || entityType === "gallery"
      ? [{ key: "photographer", label: "Photographer", current: currentData.creator, scraped: result.creator }]
      : []),
  ...
  const collectionRows = [
    ...(entityType === "group"
      ? [{ key: "aliases", label: "Aliases", current: currentData.aliases, scraped: result.aliases }]
      : []),
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 p-2">
        ...
        {result.imageUrl && (
          <img src={result.imageUrl} alt="" className="w-16 h-10 object-cover rounded flex-shrink-0" loading="lazy" />
        )}
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 text-[10px] text-muted mt-0.5">
            ...
            {entityType !== "group" && result.performerNames.length > 0 && (
              <span>{result.performerNames.length} performer(s)</span>
            )}
          </div>
        </div>
