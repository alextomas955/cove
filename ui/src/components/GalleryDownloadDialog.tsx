...
import type { DownloaderMatch, Gallery } from "../api/types";

interface Props {
  open: boolean;
  onClose: () => void;
  onNavigate: (route: any) => void;
  gallery: Pick<Gallery, "id" | "title" | "urls" | "files">;
}

export function GalleryDownloadDialog({ open, onClose, onNavigate, gallery }: Props) {
  ...
  const [resetKey, setResetKey] = useState({ open: false, id: gallery?.id, urls: gallery?.urls });
  if (resetKey.open !== open || resetKey.id !== gallery?.id || resetKey.urls !== gallery?.urls) {
    setResetKey({ open, id: gallery?.id, urls: gallery?.urls });
    if (open) {
      setUrl(gallery?.urls[0] ?? "");
      setMatches([]);
  ...
  const matchMutation = useMutation({
    ...
    mutationFn: async () => {
      ...
      return results.filter((match) => match.supportedEntity.toLowerCase() === "gallery");
    },
    onSuccess: (results) => {
      setMatches(results);
      setSelectedDownloaderId(results[0]?.downloaderId ?? "");
      setQualityId(results[0]?.qualityOptions[0]?.id ?? "");
      setError(results.length === 0 ? "No gallery downloader matched that URL." : null);
    },
  ...
  const startDownloadMutation = useMutation({
    ...
    mutationFn: async ({ queueDownload }: { queueDownload: boolean }) => {
      ...
      return { galleryId, queued: queueDownload };
    },
    onSuccess: ({ galleryId, queued }) => {
      if (queued) {
      ...
      onClose();
      onNavigate({ page: "gallery", id: galleryId });
    },
    onError: (mutationError: Error) => {
  ...
  const title = "Download Gallery Media";
  const subtitle = `Attach a downloader result to ${gallery.title || `Gallery ${gallery.id}`}.`;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="w-full max-w-2xl overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl">
          ...
          <div className="space-y-2">
            ...
            {matches.length === 0 ? (
              <div className="rounded-xl border border-dashed border-border bg-card/50 px-4 py-5 text-sm text-muted">
                Match the URL to choose a gallery downloader.
              </div>
            ) : (
              <div className="space-y-2">
                {matches.map((match) => (
            ...
            <label className="flex items-start gap-3 rounded-xl border border-border bg-card/60 px-4 py-3 text-sm text-foreground">
              ...
              <span>
                ...
                <span className="mt-1 block text-xs text-secondary">
                  Leave this off to stop Cove from creating or queueing a gallery when this source URL already has
                  downloaded files.
                </span>
              </span>
            </label>
        ...
        <div className="flex items-center justify-between border-t border-border px-5 py-4">
          <div className="text-xs text-muted">The gallery stays editable while the download job runs.</div>
