...
export function GalleriesPage({ onNavigate }: Props) {
  ...
  const { data: filteredAggregate, isLoading: filteredAggregateLoading } = useQuery({
    ...
    queryFn: () =>
      galleries.aggregate({
        ...
        objectFilter: hasObjectFilter ? (objectFilter as GalleryFilterCriteria) : undefined,
      }),
  });
  ...
  const bulkEditMut = useMutation({
    mutationFn: (values: Record<string, unknown>) => galleries.bulkUpdate({ ids: [...selectedIds], ...values } as any),
    onSuccess: () => {
  ...
  useEffect(() => {
    if (
      ...
      !listData.infiniteQuery.hasNextPage ||
      listData.infiniteQuery.isFetchingNextPage
    ) {
      return;
    }
            ...
            renderItem={(g) => (
              <GalleryTile
                ...
                onClick={(toggleOptions) =>
                  selecting ? toggle(g.id, toggleOptions) : onNavigate({ page: "gallery", id: g.id })
                }
                onNavigate={onNavigate}
          ...
          <VirtualizedWallColumns
            ...
            renderItem={(gallery) => (
              <GalleryWallCard
                ...
                onClick={(toggleOptions) =>
                  selecting ? toggle(gallery.id, toggleOptions) : onNavigate({ page: "gallery", id: gallery.id })
                }
                onNavigate={onNavigate}
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-1 left-1 right-1 z-10 flex items-end justify-between gap-2">
        ...
        {gallery.organized ? (
          <span
            className="inline-flex items-center gap-1 rounded-full bg-green-600/90 px-1.5 py-0.5 text-[10px] text-white"
            title="Organized"
          >
