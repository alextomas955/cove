    ...
    queryPage: (nextFilter) =>
      hasObjectFilter
        ? audios.findFiltered({
            ...
            objectFilter: backendObjectFilter as AudioFilterCriteria,
            filterExpression,
          })
  ...
  const { data: filteredAggregate, isLoading: filteredAggregateLoading } = useQuery({
    ...
    queryFn: () =>
      audios.aggregate({
        ...
        objectFilter:
          Object.keys(backendObjectFilter).length > 0 ? (backendObjectFilter as AudioFilterCriteria) : undefined,
        filterExpression,
      }),
  });
            ...
            renderItem={(audio) => (
              <AudioTile
                ...
                onClick={(toggleOptions) =>
                  selecting ? toggle(audio.id, toggleOptions) : onNavigate({ page: "audio", id: audio.id })
                }
                onNavigate={onNavigate}
              />
            )}
...
export function AudioCreateModal({
  ...
  const { urlDownloadMode, setUrlDownloadMode, scrapeMetadata, setScrapeMetadata } =
    useFileBackedCreatePreferences("Audio");
    ...
    mutationFn: async ({
      ...
      if (downloadMode === "now") {
        const matches = (await system.matchDownloaders({ url: requestedUrl })).filter(
          (match) => match.supportedEntity.toLowerCase() === "audio",
        );
    ...
    <>
      <EditModal title="Create Audio" open={open} onClose={onClose}>
        <FileBackedCreateSource
          ...
          filePlaceholder="C:\\Media\\audio.mp3"
          urlPlaceholder="https://example.com/audio.mp3"
        />

        <div className="grid grid-cols-2 gap-4">
          <Field label="Title">
            <TextInput value={title} onChange={setTitle} placeholder="Audio title" />
          </Field>
          <Field label="Date">
            <IsoDateInput
              value={date}
        ...
        <Field label="Studio Code">
          <TextInput value={code} onChange={setCode} placeholder="Audio code" />
        </Field>
        <Field label="Details">
          <TextArea value={details} onChange={setDetails} placeholder="Audio notes" rows={3} />
        </Field>
        <Field label="Studio">
