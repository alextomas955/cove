...
const VIDEO_FILTER_CRITERIA: CriterionDefinition[] = [
  ...VIDEO_CRITERIA,
  {
    id: "includeCompilations",
    label: "Include Compilations",
    type: "bool",
    filterKey: INCLUDE_COMPILATIONS_FILTER_KEY,
...
export function VideosPage({ onNavigate }: Props) {
  ...
  const { filter, setFilter, objectFilter, setObjectFilter, displayMode, setDisplayMode, searchMode, setSearchMode } =
    useListUrlState({
      resetKey: "videos",
  ...
  const lastPagedFilterRef = useRef<Pick<FindFilter, "page" | "perPage">>({
    page: defaultState.filter.page ?? 1,
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(
      () => setVerticalAutoScrollAwake(false),
      verticalAutoScrollEnabled ? 2600 : 3600,
    );
  ...
  const backendObjectFilter = useMemo(
    () =>
      Object.fromEntries(
        Object.entries(normalizedObjectFilter).filter(
          ([key]) => key !== INCLUDE_COMPILATIONS_FILTER_KEY && key !== FILTER_EXPRESSION_STATE_KEY,
        ),
      ),
  ...
  const compilationBlockingObjectFilter = useMemo(
    () =>
      Object.fromEntries(
        Object.entries(backendObjectFilter).filter(
          ([key, value]) => key !== IS_VR_FILTER_KEY || getBoolCriterionValue(value) !== false,
        ),
      ),
    [backendObjectFilter],
  );
  const hasCompilationBlockingObjectFilter =
    Object.keys(compilationBlockingObjectFilter).length > 0 || Boolean(filterExpression?.children.length);
  const videoVrFilterValue = getBoolCriterionValue(backendObjectFilter[IS_VR_FILTER_KEY]);
  const compilationQueryExtra = useMemo(
    () => (videoVrFilterValue === false ? { isVr: false } : undefined),
    [videoVrFilterValue],
  );
  ...
  const defaultInfiniteChunkSize =
    defaultState.filter.perPage && defaultState.filter.perPage > 0 ? defaultState.filter.perPage : 40;
  ...
  const searchModeOptions = useMemo(
    () =>
      visualSimilarityAvailable ? SEARCH_MODE_OPTIONS : SEARCH_MODE_OPTIONS.filter((mode) => mode.value === "text"),
    [visualSimilarityAvailable],
  );
  const sortOptions = useMemo(
    () =>
      visualSimilarityAvailable && searchMode === "visual"
        ? [VISUAL_MATCH_SORT_OPTION, ...VIDEO_SORT_OPTIONS]
        : VIDEO_SORT_OPTIONS,
  ...
  useEffect(() => {
    ...
    defaultState.filter.direction,
    defaultState.filter.sort,
    filter,
    searchMode,
    setFilter,
    setSearchMode,
  ...
  const handleDisplayModeChange = useCallback(
    (mode: DisplayMode) => {
      ...
      if (
        mode === "vertical" &&
        !objectFilter[VERTICAL_PORTRAIT_FILTER_KEY] &&
      ...
      if (!requiresInfinite && filter.perPage === 0) {
        ...
        setFilter({
          ...filter,
          page: lastPagedFilter.page ?? 1,
          perPage: lastPagedFilter.perPage ?? defaultState.filter.perPage,
        });
      }
    },
  ...
  const includeCompilationGroups = isIncludeCompilationGroupsEnabled(
    normalizedObjectFilter[INCLUDE_COMPILATIONS_FILTER_KEY],
  );
  const canShowCompilationGroups =
    !infinitePageSize &&
    includeCompilationGroups &&
    searchMode === "text" &&
  ...
  const defaultItems = defaultListEntries.flatMap((entry) =>
    entry.kind === "video" && entry.video ? [entry.video] : [],
  );
  ...
  const retryLoad = infinitePageSize
    ...
    : canShowCompilationGroups
      ? refetchUnified
      : refetchPage;
  const activeQueryData = infinitePageSize ? infiniteVideosQuery.data : canShowCompilationGroups ? unifiedData : data;
  ...
  useEffect(() => {
    ...
    activeVerticalVideoId,
    displayMode,
    items,
    verticalAutoScrollEnabled,
    verticalAutoScrollSeconds,
    verticalItemHeight,
  ]);
  ...
  const selectionResetKey = useMemo(
    () =>
      JSON.stringify({ filter: infiniteFilterKey, objectFilter: backendObjectFilter, filterExpression, searchMode }),
    [backendObjectFilter, filterExpression, infiniteFilterKey, searchMode],
  );
  ...
  const queryVideoQueuePage = useCallback(
  ...
  const verticalViewerStyle = verticalFullscreen
    ? undefined
    : { height: verticalViewerHeight != null ? `${verticalViewerHeight}px` : "calc(100dvh - 10rem)" };
  const verticalActiveIndex = useMemo(
    () => items.findIndex((video) => video.id === activeVerticalVideoId),
    [items, activeVerticalVideoId],
  );
  ...
  return (
    <>
      ...
      <ListPage
        title="Videos"
        metadataByline={
          !visualSearchActive && !canShowCompilationGroups ? (
        ...
        selectionMetadata={
          <MediaAggregateMetadata
            duration={selectedAggregate?.duration}
            fileSize={selectedAggregate?.fileSize}
            loading={selectedAggregateLoading}
          />
        }
      ...
      >
        {displayMode === "vertical" && (
          <>
            ...
            {infinitePageSize && (
              <div
                className="pointer-events-none fixed right-3 z-[94] sm:right-5"
                style={{ top: verticalAutoScrollTop, transform: verticalFullscreen ? "translateY(-50%)" : undefined }}
              >
                <div
                ...
                >
                  {!verticalAutoScrollAwake && (
                    <div
                      className="absolute right-0 h-12 w-1.5 rounded-l-full bg-white/70 shadow-lg"
                      aria-hidden="true"
                    />
                  )}
                  <div
                    className={`flex flex-col items-center gap-2 rounded-xl border border-white/15 bg-black/60 px-2 py-2 text-white shadow-2xl backdrop-blur transition-all duration-300 ${verticalAutoScrollAwake ? "translate-x-0 opacity-100" : "pointer-events-none translate-x-2 opacity-0"}`}
                  >
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      aria-label={
                        verticalAutoScrollEnabled ? "Pause vertical auto-scroll" : "Start vertical auto-scroll"
                      }
                    ...
                    />
                    <span className="text-[10px] text-white/80 tabular-nums [writing-mode:vertical-lr]">
                      {verticalAutoScrollSeconds}s/item
                    </span>
                  </div>
                </div>
              </div>
              ...
              renderItem={({ item: video }) => (
                <VideoFeedCard
                  ...
                  onPlaybackEligibilityChange={
                    defaultFeedVideoSound
                      ? (eligible) =>
                          setFeedAudioVideoId((current) =>
                            eligible ? video.id : current === video.id ? null : current,
                          )
                      : undefined
                  }
          ...
          (infinitePageSize ? (
            ...
            <EntityCardGrid minCardWidth="var(--card-min-width, 200px)">
              {listEntries.map((entry) =>
                entry.kind === "compilation" && entry.group ? (
                  <CompilationGroupCard
  ...
  return (
    <div className="entity-card relative flex h-full cursor-pointer flex-col overflow-hidden rounded border border-border bg-card transition-colors hover:border-accent/60 group">
      <RouteCardLinkOverlay
        route={{ page: "compilation", id: group.id }}
        onClick={() => onNavigate({ page: "compilation", id: group.id })}
        label={`Play compilation ${group.name}`}
        selectionSafeZone
      />
...
function VideoWallCard({
  ...
  return (
    ...
    >
      ...
      />
      <div
        className={`absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent transition-opacity ${showTitle ? "opacity-0 group-hover:opacity-100" : "opacity-0"}`}
      />
      {showTitle ? (
...
function VideoFeedCard({
  ...
  const ratingMut = useMutation({
    mutationFn: (value: number | undefined) =>
      entityEngagement.setRating("video", video.id, { value: value ?? null, aspect: "overall" }),
    ...
    <FeedCardFrame
      ...
      headerActions={
        <>
          <FeedInlineRating
            value={ratingValue}
            onChange={(value) => ratingMut.mutate(value)}
            readOnly={!canEngage}
            pending={ratingMut.isPending}
          />
          <FeedActionPill>
        ...
        <>
          {video.performers.slice(0, 4).map((performer) => (
            <FeedChipButton
              ...
              onClick={(event) =>
                selecting
                  ? onSelect?.(toggleOptionsFromEvent(event))
...
function VideoFeedVideoControls({
  controls,
  soundEnabled,
  onToggleSound,
}: {
  controls: WallMediaVideoControlsState;
  soundEnabled: boolean;
  onToggleSound: () => void;
}) {
...
function VideoVerticalViewerCard({
  video,
  feedVideoSource,
  useVideo,
  soundEnabled,
  ...
  const openOrSelect = (toggleOptions?: MultiSelectToggleOptions) =>
    selecting ? onSelect?.(toggleOptions) : onNavigate(video.id);

  return (
    <article
      data-vertical-video-id={video.id}
      className={`flex h-full min-h-0 snap-start snap-always items-center justify-center ${fullscreen ? "px-0 py-0" : "px-2 py-0 sm:px-4"}`}
    >
      <WallMediaCard
        title={title}
        imageSrc={coverUrl}
        ...
        playbackTracking={{
          ...
          hostId: video.id,
          surface: "vertical",
          scopeKey: `video-vertical:${video.id}`,
        }}
        aspectRatio="9 / 16"
        imageClassName="object-cover"
      ...
      >
        ...
        {duration > 0 ? (
          <span className="absolute right-2 top-12 rounded bg-black/65 px-2 py-0.5 text-xs text-white">
            {formatDuration(duration)}
          </span>
        ) : null}
