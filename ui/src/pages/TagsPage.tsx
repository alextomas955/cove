...
export function TagsPage({ onNavigate }: Props) {
  const defaultState = useMemo(() => {
    ...
    return {
      ...
      displayMode: resolveSavedDisplayMode(
        ...
        ["grid", "list", "graph", "tagger"] as const,
        "grid",
      ) as DisplayMode,
    };
  }, []);
  ...
  const graphFindFilter = useMemo(() => ({ ...filter, page: 1, perPage: GRAPH_VIEW_LIMIT }), [filter]);
  ...
  const selectionItems: Array<Pick<Tag, "id" | "name" | "imagePath">> =
    displayMode === "graph" ? (graphData?.items ?? []) : items;
  const selectionResetKey = useMemo(
    () => JSON.stringify({ filter: listData.infiniteFilterKey, objectFilter, displayMode }),
    [displayMode, listData.infiniteFilterKey, objectFilter],
  );
  const { selectedIds, toggle, selectAll, selectIds, selectNone, invertSelection } = useMultiSelect(selectionItems, {
    preserveOnItemsChange: displayMode !== "graph" && listData.infinitePageSize,
  ...
  return (
    <>
      ...
      <ListPage
        ...
        isLoading={isLoading}
        error={
          displayMode === "graph" ? (!graphData && graphError instanceof Error ? graphError : null) : listData.loadError
        }
        onRetry={() => {
          void (displayMode === "graph" ? refetchGraph() : listData.refetch());
        }}
              ...
              renderItem={(tag) => (
                <TagTile
                  ...
                  onClick={(toggleOptions) =>
                    selecting ? toggle(tag.id, toggleOptions) : onNavigate({ page: "tag", id: tag.id })
                  }
                  onNavigate={onNavigate}
      ...
      <ConfirmDialog
        ...
        title="Delete Tag"
        message={
          graphDeleteTarget ? `Delete tag "${graphDeleteTarget.name}"? This cannot be undone.` : "Delete this tag?"
        }
      ...
      <MergeDialog
        ...
        items={selectionItems
          ...
          .map((t) => ({ id: t.id, name: t.name, imagePath: t.imagePath }))}
        onMerge={tags.merge}
        queryKey="tags"
      />
    </>
...
export function TagCreateModal({
  ...
  const [form, setForm] = useState({
    ...
    aliases: [] as string[],
    color: "",
    tagGroupId: undefined as number | undefined,
    minOccurrenceSec: undefined as number | undefined,
    minOccurrencePercent: undefined as number | undefined,
  });
  ...
  const resetForm = () => {
    setForm({
      ...
      minOccurrenceSec: undefined,
      minOccurrencePercent: undefined,
    });
    ...
    <EditModal title="Create Tag" open={open} onClose={handleClose}>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <Field label="Min Seconds">
          <NumberInput
            value={form.minOccurrenceSec}
            onChange={(value) => setForm({ ...form, minOccurrenceSec: value })}
            min={0}
          />
        </Field>
        <Field label="Min Percent">
          <NumberInput
            value={form.minOccurrencePercent}
            onChange={(value) => setForm({ ...form, minOccurrencePercent: clampOptionalPercent(value) })}
            min={0}
            max={100}
          />
        </Field>
