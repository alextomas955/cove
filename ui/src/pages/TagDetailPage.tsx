...
export function TagDetailPage({ id, onNavigate }: Props) {
    ...
    orderDetailTabsByMenuItems(
      [
        { key: "videos", label: "Videos", count: includeSubTags ? recursiveTag?.videoCount : tag?.videoCount },
        {
          key: "performers",
          label: "Performers",
          count: includeSubTags ? recursiveTag?.performerCount : tag?.performerCount,
        },
        { key: "images", label: "Images", count: includeSubTags ? recursiveTag?.imageCount : tag?.imageCount },
        {
          key: "galleries",
          label: "Galleries",
          count: includeSubTags ? recursiveTag?.galleryCount : tag?.galleryCount,
        },
  ...
  const visibleTagTabs = filterItemsByPermission(
    tagTabs,
    {
      videos: "videos.read",
  ...
  const {
    ...
    setFavorite: setTagFavorite,
    rating: tagRating,
    setRating: setTagRating,
  } = useEntityEngagement("tag", id, {
      ...
      () => [
        {
          ...
          action: () => {
            if (canWriteTag) setEditing((value) => !value);
          },
        },
        {
          id: "detail.favorite",
          ...
          surface: "detail" as const,
          action: () => {
            if (tag && canEngageTag) setTagFavorite(!tagFavorite);
          },
        },
      ],
  ...
  const updateMut = useMutation({
    ...
    onMutate: async (data) => {
      ...
      queryClient.setQueryData<TagDetailModel>(["tag", id], (current) =>
  ...
  return (
    <>
      <EntityHeroLayout
        ...
        title={
          <FieldProvenanceHover fieldProvenance={tag.fieldProvenance} fieldKey="name">
            {tag.name}
          </FieldProvenanceHover>
        }
        sortName={
          tag.sortName && tag.sortName !== tag.name ? (
            <FieldProvenanceHover fieldProvenance={tag.fieldProvenance} fieldKey="sortName">
              {tag.sortName}
            </FieldProvenanceHover>
          ) : undefined
        }
        aliases={
          tag.aliases.length > 0 ? (
            <FieldProvenanceHover fieldProvenance={tag.fieldProvenance} fieldKey="aliases">
              {tag.aliases.join(", ")}
            </FieldProvenanceHover>
          ) : undefined
        }
        description={
          tag.description ? (
            <FieldProvenanceHover fieldProvenance={tag.fieldProvenance} fieldKey="description" block>
              <NarrativeText>{tag.description}</NarrativeText>
            </FieldProvenanceHover>
          ) : undefined
        }
        favorite={tagFavorite}
        ...
        actions={
          <>
            ...
            {canWriteTag ? (
              <button onClick={() => setEditing(true)} className={HERO_PRIMARY_ACTION_BUTTON_CLASS}>
                ...
                >
                  {canWriteTag ? (
                    <button
                      ...
                      className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-secondary hover:bg-card hover:text-foreground"
                    >
                  ...
                  {canDeleteTag ? (
                    <button
                      ...
                      className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-red-300 hover:bg-red-950/30"
                    >
      ...
      <DetailMergeDialog
        ...
        targetItem={{
          id: tag.id,
          name: tag.name,
          imagePath: tagImageUrl,
          subtitle: tag.sortName && tag.sortName !== tag.name ? tag.sortName : undefined,
        }}
          ...
          <div className="flex min-w-0 flex-wrap items-center gap-1.5">
            ...
            {tag.parents.map((parent) => (
              <TagBadge
                ...
                name={parent.name}
                tag={parent}
                provenance={resolveTagProvenance(parent, tag.fieldProvenance, "parents")}
                onClick={() => onNavigate({ page: "tag", id: parent.id })}
              />
            ))}
          </div>
        ) : null}
          ...
          <div className="flex min-w-0 flex-wrap items-center gap-1.5">
            ...
            {tag.children.map((child) => (
              <TagBadge
                ...
                name={child.name}
                tag={child}
                provenance={resolveTagProvenance(child, tag.fieldProvenance, "children")}
                onClick={() => onNavigate({ page: "tag", id: child.id })}
              />
            ))}
          </div>
        ) : null}
...
function TagVideosPanel({
  ...
  const queryPage = useCallback(
    ...
    [hasObjectFilter, includeSubTags, objectFilter, tagId],
  );
  const {
    data,
    isLoading,
  ...
  return (
    <ListQueryState
      ...
      loading={<LoadingPanel icon={<Film className="h-10 w-10" />} message="Loading videos..." />}
      empty={<EmptyPanel icon={<Film className="h-12 w-12" />} message="No videos with this tag" />}
    >
      <>
...
function TagPerformersPanel({
  ...
  return (
    <ListQueryState
      ...
      loading={<LoadingPanel icon={<UserRound className="h-10 w-10" />} message="Loading performers..." />}
      empty={<EmptyPanel icon={<UserRound className="h-12 w-12" />} message="No performers with this tag" />}
    >
      <>
...
function TagImagesPanel({
  ...
  return (
    <ListQueryState
      ...
      loading={<LoadingPanel icon={<ImageIcon className="h-10 w-10" />} message="Loading images..." />}
      empty={<EmptyPanel icon={<ImageIcon className="h-12 w-12" />} message="No images with this tag" />}
    >
      <>
        <ContextualImageListView
          ...
          totalCount={data.totalCount}
          queryPage={queryPage}
          interactionSource="tagDetailPage"
          interactionMeta={{ tagId }}
          displayMode={displayMode}
          zoomLevel={zoomLevel}
          selectedIds={selectedIds}
          selecting={selecting}
...
function TagGalleriesPanel({
  ...
  return (
    <ListQueryState
      ...
      loading={<LoadingPanel icon={<FolderOpen className="h-10 w-10" />} message="Loading galleries..." />}
      empty={<EmptyPanel icon={<FolderOpen className="h-12 w-12" />} message="No galleries with this tag" />}
    >
      <>
        <RelatedEntityListView
          entityType="galleries"
...
function TagAudiosPanel({
  ...
  return (
    <ListQueryState
      ...
      loading={<LoadingPanel icon={<Headphones className="h-10 w-10" />} message="Loading audios..." />}
      empty={<EmptyPanel icon={<Headphones className="h-12 w-12" />} message="No audios with this tag" />}
    >
      <>
        <RelatedEntityListView
          entityType="audios"
...
function TagTextsPanel({
  ...
  return (
    <ListQueryState
      ...
      loading={<LoadingPanel icon={<FileText className="h-10 w-10" />} message="Loading texts..." />}
      empty={<EmptyPanel icon={<FileText className="h-12 w-12" />} message="No texts with this tag" />}
    >
      <>
        <RelatedEntityListView
          entityType="texts"
...
function TagSegmentsPanel({
  ...
  const { filter, setFilter, objectFilter, setObjectFilter, displayMode, setDisplayMode, availableDisplayModes } =
    useRelatedDetailListUrlState({
      stateKey: "segments",
      resetKey: "tag-segments",
      entityType: "segments",
      builtInFilter: { page: 1, perPage: 24, direction: "desc", sort: "updated_at" },
      defaultFilterKey: "rawsegments",
    });
  ...
  const segmentCriteria = useMemo(
    () =>
      createSegmentCriteria({
        sourceOptions: (sourceOptionsQuery.data ?? []).map((option) => ({
          value: option.value,
          label: `${option.value} (${option.count})`,
        })),
        kindOptions: (kindOptionsQuery.data ?? []).map((option) => ({
          value: option.value,
          label: `${option.value} (${option.count})`,
        })),
      }),
  ...
  const {
    ...
    fetchAllIds,
    loadMore,
  } = useDetailListQuery<SegmentRecord>({
    ...
    <DetailListToolbar
      ...
      selectionActions={
        canEditSegments ? (
          <button
            type="button"
            onClick={() => removeTagMut.mutate()}
            disabled={selectedIds.size === 0 || removeTagMut.isPending}
            className="flex items-center gap-1 rounded px-2 py-0.5 text-xs text-red-300 hover:bg-red-900/20 hover:text-red-200 disabled:cursor-not-allowed disabled:opacity-50"
          >
  ...
  return (
    <ListQueryState
      ...
      loading={<LoadingPanel icon={<Layers className="h-10 w-10" />} message="Loading segments..." />}
      empty={<EmptyPanel icon={<Layers className="h-12 w-12" />} message="No segments with this tag" />}
    >
      <>
        <RelatedEntityListView
          entityType="segments"
...
function TagStudiosPanel({
    ...
    queryFn: (nextFilter) =>
      hasObjectFilter || includeSubTags
        ? studios.findFiltered({
            findFilter: nextFilter,
            objectFilter: withRequiredMultiId(
              objectFilter as StudioFilterCriteria,
              "tagsCriterion",
              tagId,
              includeSubTags ? -1 : undefined,
            ),
  ...
  return (
    <ListQueryState
      ...
      loading={<LoadingPanel icon={<Building2 className="h-10 w-10" />} message="Loading studios..." />}
      empty={<EmptyPanel icon={<Building2 className="h-12 w-12" />} message="No studios with this tag" />}
    >
      <>
        <RelatedEntityListView
          entityType="studios"
...
function TagGroupsPanel({
  ...
  return (
    <ListQueryState
      ...
      loading={<LoadingPanel icon={<Layers className="h-10 w-10" />} message="Loading groups..." />}
      empty={<EmptyPanel icon={<Layers className="h-12 w-12" />} message="No groups with this tag" />}
    >
      <>
        <RelatedEntityListView
          entityType="groups"
