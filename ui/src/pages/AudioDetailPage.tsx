...
const MediaDownloadDialog = lazy(() =>
  import("../components/MediaDownloadDialog").then((module) => ({ default: module.MediaDownloadDialog })),
);
...
export function AudioDetailPage({ id, onNavigate }: Props) {
  ...
  const canDownloadAudio =
    canWriteAudio && hasPermission("jobs.run") && (audio?.files.length ?? 0) === 0 && (audio?.urls.length ?? 0) > 0;
    ...
    return (
      [
        audio.performers
          ...
          .join(", "),
        audio.studioName,
        audio.date ? formatDate(audio.date) : null,
      ]
        .filter(Boolean)
        .join(" • ") || undefined
    );
  ...
  if (isApiNotFoundError(audioLoadError)) {
    return (
      <div className="rounded-3xl border border-dashed border-border bg-card/70 px-6 py-10 text-sm text-muted">
        Audio #{id} was not found.
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <MediaDetailLayout
        title={
          <FieldProvenanceHover fieldProvenance={audio.fieldProvenance} fieldKey="title">
            {displayTitle}
          </FieldProvenanceHover>
        }
        subtitle={
          <MediaStudioSubtitle
            ...
            canReadStudio={canReadStudio}
          />
        }
        backLabel={backLabel}
        onGoBack={goBack}
        headerImage={
          <StudioHeaderImage studioId={audio.studioId} studioName={audio.studioName} onNavigate={onNavigate} />
        }
        media={audioMedia}
        ...
        engagement={{
          primaryContent: (
            <InteractiveRating
              value={audioRating}
              onChange={(value) => setAudioRating(value)}
              readOnly={!canEngageAudio}
            />
          ),
          ...
          <>
            ...
            {canWriteAudio ? (
              <button
                type="button"
                onClick={() => {
                  if (!updateAudioMut.isPending) updateAudioMut.mutate({ organized: !audio.organized });
                }}
              ...
              <span
                className="inline-flex items-center justify-center rounded bg-green-600 p-1 text-white"
                title="Organized"
              >
                <Check className="h-4 w-4" />
              </span>
              ...
              <div className="relative" ref={opsMenuRef}>
                ...
                >
                  ...
                  {(audio.files?.length ?? 0) > 0 && canLibraryScan ? (
                    ...
                    >
                      <RefreshCw
                        className={["h-3.5 w-3.5", rescanAudioMut.isPending ? "animate-spin" : ""].join(" ")}
                      />{" "}
                      Rescan
                    </button>
                  ) : null}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {audio.tags.map((tag) => (
                      <TagBadge
                        ...
                        name={tag.name}
                        tag={tag}
                        provenance={resolveTagProvenance(tag, audio.fieldProvenance)}
              ...
              {canReadPerformers && audio.performers.length > 0 ? (
                <div>
                  ...
                  <FieldProvenanceHover fieldProvenance={audio.fieldProvenance} fieldKey="performers" block>
                    <div
                      className={audio.performers.length > 1 ? "grid grid-cols-2 gap-3" : "grid max-w-[220px] gap-3"}
                    >
            ...
            <section className="rounded-3xl border border-border bg-card/75 p-4">
              <div className="space-y-3">
                {audio.tracks.map((track) => (
                  <div
                    key={track.id}
                    className="flex items-center justify-between gap-3 rounded-2xl border border-border/80 bg-background/75 px-4 py-3 text-sm"
                  >
                    <div className="min-w-0">
                      <div className="font-medium text-foreground">
                        {track.title?.trim() || `Track ${track.orderIndex + 1}`}
                      </div>
                ...
                <MediaDetailLayout.Metadata key={file.id}>
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      ...
                      <span className="rounded-full border border-border px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.18em] text-muted">
                        {file.format}
                      </span>
                    </div>
                  </div>
                  <DetailGrid
                    items={[
                      ...
                      {
                        label: "Bitrate",
                        value: file.bitRate > 0 ? `${Math.round(file.bitRate / 1000)} kbps` : undefined,
                      },
                      {
                        label: "Sample Rate",
                        value: file.sampleRate ? `${Intl.NumberFormat().format(file.sampleRate)} Hz` : undefined,
                      },
...
function DetailGrid({
  ...
  items: { label: string; value?: string; fieldKey?: string | string[] }[];
      ...
      {visibleItems.map((item) => (
        <div key={item.label}>
          ...
          <dd className="mt-1 text-sm text-foreground">
            {item.fieldKey ? (
              <FieldProvenanceHover fieldProvenance={fieldProvenance} fieldKey={item.fieldKey}>
                {item.value}
              </FieldProvenanceHover>
            ) : (
              item.value
    ...
    <div className="space-y-6 text-sm">
      <section>
        ...
        <div className="mb-2 grid grid-cols-2 gap-2">
          ...
          <div>
            <span className="text-muted">Listened:</span>{" "}
            <span className="text-foreground">{formatDuration(playDuration)}</span>
          </div>
          <div>
            <span className="text-muted">Page Visits:</span> <span className="text-foreground">{pageVisitCount}</span>
          </div>
          {history ? (
            <div>
              <span className="text-muted">Distinct Listened:</span>{" "}
              <span className="text-foreground">{formatDuration(history.totalDistinctWatchedSec ?? 0)}</span>
            </div>
          ) : null}
        </div>
      </section>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <span className="text-muted">Created:</span> <span className="text-foreground">{formatDate(createdAt)}</span>
        </div>
        <div>
