...
const ImageCreateModal = lazy(() =>
  import("./ImageEditModal").then((module) => ({ default: module.ImageCreateModal })),
);
...
export function ImagesPage({ onNavigate }: Props) {
  const defaultState = useMemo(() => {
    ...
    return {
      ...
      displayMode: resolveSavedDisplayMode(
        ...
        ["grid", "list", "wall", "tagger", "feed"] as const,
        "grid",
      ) as DisplayMode,
    };
  }, []);
  ...
  const sortOptions = useMemo(
    () =>
      visualSimilarityAvailable && searchMode === "visual"
        ? [VISUAL_MATCH_SORT_OPTION, ...IMAGE_SORT_OPTIONS]
        : IMAGE_SORT_OPTIONS,
  ...
  const queryImagesPage = useCallback(
  ...
  const wallColumnOptions = useMemo(
    () => ({ stable: infinitePageSize, getKey: (image: Image) => image.id }),
    [infinitePageSize],
  );
  const wallColumns = useWallColumns(items, wallColumnCount, estimateImageWallHeight, wallColumnOptions);
  const selectionResetKey = useMemo(
    () => JSON.stringify({ filter: listData.infiniteFilterKey, objectFilter, searchMode }),
    [listData.infiniteFilterKey, objectFilter, searchMode],
  );
          ...
          <VirtualizedWallColumns
            ...
            renderItem={(img) => (
              <ImageWallCard
                image={img}
                onClick={(toggleOptions) =>
                  selecting ? toggle(img.id, toggleOptions) : onNavigate({ page: "image", id: img.id })
                }
                selected={selectedIds.has(img.id)}
                selecting={selecting}
                onSelect={(toggleOptions) => toggle(img.id, toggleOptions)}
...
function ImageFeedCard({
  ...
  const ratingMut = useMutation({
    mutationFn: (value: number | undefined) =>
      entityEngagement.setRating("image", image.id, { value: value ?? null, aspect: "overall" }),
