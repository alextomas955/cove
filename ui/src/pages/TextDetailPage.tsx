...
function SourcePdfViewer({
  ...
  return (
    <div className="flex min-h-[70svh] flex-1 flex-col overflow-hidden rounded-lg border border-border bg-surface">
      ...
      <iframe title={title} src={frameUrl} className="min-h-0 flex-1 border-0 bg-surface" scrolling="yes" />
    </div>
  );
}
...
export function TextDetailPage({ id, onNavigate }: Props) {
  ...
  const canDownloadTextMedia =
    canWriteText && hasPermission("jobs.run") && (text?.files.length ?? 0) === 0 && (text?.urls.length ?? 0) > 0;
  ...
  if (isApiNotFoundError(textLoadError)) {
    return (
      <div className="rounded-3xl border border-dashed border-border bg-card/70 px-6 py-10 text-sm text-muted">
        Text document #{id} was not found.
      </div>
    );
  }
  ...
  const textMedia = (
    <div className="flex min-h-0 flex-1 bg-background p-2 sm:p-4">
      {showSourcePdf ? (
        <SourcePdfViewer
          title={displayTitle}
          sourceUrl={texts.fileUrl(text.id)}
          pageCount={text.maxPageCount ?? primaryFile?.pageCount}
        />
      ) : contentLoading ? (
  ...
  return (
    <>
      ...
      <MediaDetailLayout
        title={
          <FieldProvenanceHover fieldProvenance={text.fieldProvenance} fieldKey="title">
            {displayTitle}
          </FieldProvenanceHover>
        }
        subtitle={
        ...
        onGoBack={goBack}
        headerImage={
          <StudioHeaderImage studioId={text.studioId} studioName={text.studioName} onNavigate={onNavigate} />
        }
        media={textMedia}
        ...
        engagement={{
          primaryContent: (
            <InteractiveRating
              value={textRating}
              onChange={(value) => setTextRating(value)}
              readOnly={!canEngageText}
            />
          ),
          ...
          additionalMetrics: [
            {
              ...
              icon: (
                <ThumbsUp
                  className={["h-4 w-4", (textEngagement?.likeCount ?? 0) > 0 ? "fill-accent text-accent" : ""].join(
                    " ",
                  )}
                />
              ),
            ...
            {
              label: "Words",
              value: text.maxWordCount ? Intl.NumberFormat().format(text.maxWordCount) : "-",
          ...
          <>
            ...
            {canWriteText ? (
              <button
                type="button"
                onClick={() => {
                  if (!updateTextMut.isPending) updateTextMut.mutate({ organized: !text.organized });
                }}
              ...
              <div className="relative" ref={opsMenuRef}>
                ...
                >
                  ...
                  {(text.files?.length ?? 0) > 0 && canLibraryScan ? (
                    ...
                    >
                      <RefreshCw className={["h-3.5 w-3.5", rescanTextMut.isPending ? "animate-spin" : ""].join(" ")} />{" "}
                      Rescan
                    </button>
                  ) : null}
                  {canGenerateText ? (
              ...
              <div>
                <DetailGrid
                  ...
                  items={[
                    {
                      label: "Words",
                      value: text.maxWordCount ? Intl.NumberFormat().format(text.maxWordCount) : undefined,
                    },
                  ...
                  <div className="flex flex-wrap gap-2">
                    {text.tags.map((tag) => (
                      <TagBadge
                        ...
                        name={tag.name}
                        tag={tag}
                        provenance={resolveTagProvenance(tag, text.fieldProvenance)}
                ...
                <MediaDetailLayout.Metadata key={file.id}>
                  ...
                  <DetailGrid
                    items={[
                      ...
                      {
                        label: "Words",
                        value: file.wordCount ? Intl.NumberFormat().format(file.wordCount) : undefined,
                      },
    ...
    <div className="space-y-6 text-sm">
      <section>
        ...
        <div className="mb-2 grid grid-cols-2 gap-2">
          ...
          <div>
            <span className="text-muted">Time Open:</span>{" "}
            <span className="text-foreground">{formatDuration(timeOpen)}</span>
          </div>
        </div>
      </section>
