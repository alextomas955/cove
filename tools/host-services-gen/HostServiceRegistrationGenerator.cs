...
public sealed class HostServiceRegistrationGenerator : IIncrementalGenerator
{
    ...
    private static readonly DiagnosticDescriptor DuplicateInterfaceExposure = new(
        id: "COVE0002",
        title: "Interface exposed by more than one concrete type",
        messageFormat: "'{0}' is exposed by more than one concrete type; the last registration wins and the others are silently shadowed",
        category: "Cove.HostServices",
    ...
    private static void Emit(SourceProductionContext context, ImmutableArray<EquatableArray<ExposedService>> collected)
    {
        ...
        // Two different concrete types (or one type carrying the attribute twice) exposed as the same
        // interface both emit a registration for it; DI keeps only the last, silently shadowing the
        // rest. Warn at every offending exposure so the ambiguity is visible at build time.
        var interfaceExposureCounts = new Dictionary<string, int>(StringComparer.Ordinal);
        foreach (var service in conforming)
        {
            interfaceExposureCounts.TryGetValue(service.InterfaceFqn, out var count);
            interfaceExposureCounts[service.InterfaceFqn] = count + 1;
        }

        foreach (var service in conforming)
        {
            if (interfaceExposureCounts[service.InterfaceFqn] > 1)
            {
                context.ReportDiagnostic(Diagnostic.Create(
                    DuplicateInterfaceExposure,
                    service.Location,
                    service.InterfaceFqn));
            }
        }
