...
- Conversion aims at the bitrate a video's resolution and frame rate can actually make use of, rather than at a fixed quality setting. Because of that it can tell you before starting what the result will be: a video already at or below that bitrate is left alone instead of being re-encoded for nothing, and one that would only shrink slightly is skipped unless you ask for it. Hardware encoding is used where a supported GPU is available, and is offered as its own choice because it trades some quality per megabyte for a large amount of speed.
