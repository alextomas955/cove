...
**Text** search covers videos and related metadata such as tags and performers. See [Video text search](../video-text-search/) for the complete list of searched values, matching rules, relevance order, and differences from global or visual search. **Visual** search and its **Visual Match** sort appear only when a visual-similarity provider is available.
...
| **Edit** | Bulk-edits rating, organized state, VR state, studio, date, studio code, director, tags, performers, groups, or custom field values. Multi-value relationships can be added, removed, or replaced. Custom fields share one Overwrite, Add, or Remove mode per apply, and each field can instead be cleared. |
