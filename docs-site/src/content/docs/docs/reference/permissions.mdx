...
A read permission authorizes reading entities of its kind and writing the principal's own personal state about them: bookmarks, favorites, ratings, and playback history. Personal state is per-user and invisible to others, so recording it does not require the write permission that changing the shared record would. See [Personal state](../personal-state/).
