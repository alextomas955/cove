...
The settings page issues unscoped tokens. To choose a scope, use [Cove CLI](https://github.com/yourcove/cove/tree/main/cli), which warns before it sends a scope the current identity does not hold:
...
`viewer` is a read-only preset; a scope otherwise lists permission keys explicitly, because a resource wildcard such as `videos.*` is not a registered permission and Cove rejects it. The superuser key `*` is registered, so an owner can scope a token to it, but that is the same as issuing the token with no scope at all.

Cove expands implied permissions inside a scope, so a key also grants what it implies: a scope of `images.delete.file` reaches `images.delete` and `images.read`. The expansion is still intersected with the owner's permissions, so it cannot escalate. A rejected scope is answered with 403 and names the permissions that caused the rejection, separating keys Cove does not know from keys the owner does not hold.
...
The global MVC permission filter does not wrap extension minimal-API delegates. Instead, Cove evaluates the endpoint conventions supplied by `Cove.Sdk` before invoking an extension route. Use `RequireCovePermission` and, when applicable, `RequireCoveEntityAccess`; use `AllowWithoutCovePermission` or `AllowCoveAnonymous` only when that access is intentional.

For compatibility with extensions built before these conventions existed, an endpoint without Cove authorization metadata remains anonymous and produces a registration warning. Do not rely on that fallback for new endpoints. See [Extension permissions](../../extensions/permissions/) for the full policy behavior and examples.
